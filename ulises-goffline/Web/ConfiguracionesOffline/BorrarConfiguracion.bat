...
set TituloProceso=Borrar Conf. SCV %date% %time%
title %TituloProceso%
REM ----------------------------------------------------------------------------
...
REM     4. Nombre de la Configuración a borrar
...
call :LogScriptPPAL ""
call :LogScriptPPAL "  SCRIPT PARA BORRAR UNA CONF. OFFLINE SCV"
call :LogScriptPPAL "__________________________________________________________________________"
call :LogScript ""
...
call :LogScript "   Borrar conf. %BD_CD40_Borrar%:"
...
call :AddLogProceso "Eliminando conf. %BD_CD40_Borrar% ..." || goto :FIN_ERROR

call :UpdLogProceso "Comprobando conf. %BD_CD40_Borrar%" || goto :FIN_ERROR
call :CheckBD_Existe %BD_CD40_Borrar% || goto :FIN_ERROR

call :Check_No_Online  %BD_CD40_Borrar% || goto :FIN_ERROR

:BORRAR_CONF
call :UpdLogProceso "Eliminando conf. %BD_CD40_Borrar%" || goto :FIN_ERROR
...
call :ComprobarErrores "Eliminando conf." "%ErrFile%" || goto :FIN_ERROR
...
:CheckBD_Existe
  ...
  if %NumBD% NEQ 1 (
    call :LogScript ""
    call :LogScript "    %Codigo_Error%: La BD %BD% no está en la lista de CONFIGURACIONES"
  )
  ...
  if %NumBD% NEQ 1 (
    call :LogScript ""
    call :LogScript "    %Codigo_Error%: La BD %BD% no existe"
  )
...
REM ---------------------------------------------------------------------------
REM Check_No_Online
REM Comprueba que la BD no es Online
REM ---------------------------------------------------------------------------
:Check_No_Online
  set BD=%~1
  ...
  call :ComprobarErrores "Comprobando si se trata de la BD Online" "%ErrFile%" || goto :FinErr_Check_No_Online
  ...
  if %NumBD% NEQ 0 (
    call :LogError "Se trata de la conf Online, no se puede borrar desde este script"
	goto :FinErr_Check_No_Online
  )

:Fin_Check_No_Online
  goto :eof
:FinErr_Check_No_Online
  EXIT /B 1
...
call :LogScript "      4. Nombre de la Configuración a borrar"
...
call :LogScriptPPAL " ________________________________________________________________________"
call :LogScriptPPAL ""
call :LogScriptPPAL "  CONF. BORRADA ^(!Duracion!^)"
call :LogScriptPPAL " ________________________________________________________________________"
call :LogScriptPPAL "
