...
set TituloProceso=Sincronizar Conf. SCV %date% %time%
title %TituloProceso%
REM ----------------------------------------------------------------------------
...
if [%6] == [] goto :FIN_ERROR_PARAMETROS

Set GUID=%1
Set Usuario=%2
Set Password=%3
Set BD_CD40=%4
Set Servidor_Origen=%5
Set Servidor_Destino=%6
...
call :LogScriptPPAL ""
call :LogScriptPPAL "  SCRIPT PARA SINCRONIZAR LA CONFIGURACIÓN SCV"
call :LogScriptPPAL "__________________________________________________________________________"
call :LogScript ""
...
if "%ProcesoPPAL%" NEQ "%ProcesoActual%" (
  call :AddLogProceso "Sincronizando %BD_CD40% ..." || goto :FIN_ERROR
) else (
  call :AddLogProceso "Sincronizando %BD_CD40% de %Servidor_Origen% a %Servidor_Destino% ..." || goto :FIN_ERROR
)

call :DescConf_Origen

call :LogScript "   Sincronizar %BD_CD40%"
call :LogScript "      Usuario         : %Usuario%"
call :LogScript "      Servidor_Origen : %Servidor_Origen%"
call :LogScript "      Servidor_Destino: %Servidor_Destino%"
if NOT [%ServidorLogProcesos%] == [%Servidor%] (
  call :LogScript "      ServidorLog     : %ServidorLogProcesos%"
)

call :UpdLogProceso "Exportando %BD_CD40% de %Servidor_Origen%" || goto :FIN_ERROR
call ExportarConfiguracion.bat %GUID% %Usuario% %Password% %BD_CD40% "-- Version temporal %ProcesoActual%" "TODO" "%DumpTempFile%" %Servidor_Origen%  2> %ErrFile%
call :ComprobarErrores "Exportando %BD_CD40% de %Servidor_Origen%" "%ErrFile%" || goto :FIN_ERROR

call :UpdLogProceso "Importando %BD_CD40% en %Servidor_Destino%" || goto :FIN_ERROR
call ImportarConfiguracion.bat %GUID% %Usuario% %Password% %BD_CD40% "%BD_CD40_Desc%" 1 "%DumpTempFile%" %Servidor_Destino% 2> %ErrFile%
call :ComprobarErrores "Importando %BD_CD40% en %Servidor_Destino%" "%ErrFile%" || goto :FIN_ERROR
...
REM ---------------------------------------------------------------------------
REM DescConf_Origen: Descripción de la conf Origen
REM ---------------------------------------------------------------------------
:DescConf_Origen
  mysql --defaults-file=%InfoConexion% -h %Servidor_Origen% -f -n -N -B -e "SELECT DESCRIPCION FROM new_cd40_offline.CONFIGURACIONES WHERE NOMBRE = '%BD_CD40%'" > %SQLTempFile% 2> %ErrFile%
  call :ComprobarErrores "Consultando desc. la conf. offline orgen" "%ErrFile%" || goto :FinErr_DescConf_Origen
  
  REM El log incluirá la descripción que le corresponde a la configuración
  set /a NumBD=0
  ...
  if %NumBD% NEQ 1 (
	set BD_CD40_Desc=Desc. %BD_CD40%
	goto :FinErr_DescConf_Origen
  )

:Fin_DescConf_Origen
  goto :eof
:FinErr_DescConf_Origen
  EXIT /B 1
...
call :LogScript "      4. BD a Sincronizar"
call :LogScript "      5. Servidor Origen"
call :LogScript "      6. Servidor Destino"
...
call :LogScriptPPAL " ________________________________________________________________________"
call :LogScriptPPAL ""
call :LogScriptPPAL "  SINCRO. CONF. FINALIZADA ^(!Duracion!^)"
call :LogScriptPPAL " ________________________________________________________________________"
call :LogScriptPPAL "
