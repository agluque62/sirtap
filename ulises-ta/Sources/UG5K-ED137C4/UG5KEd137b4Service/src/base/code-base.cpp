...
void CodeBase::_Log(plog::Severity level, const char *from, int line, const char *fmt, va_list args)
{
	...
	if (csize < mcola) {
		...
		Tools::fatalerror(string("Cola de Log llena... Tama�o Maximo: ") + Tools::itoa(mcola));
		hysteresis = 1;
	}
}
...
void CodeBase::_FormatLog(plog::Severity level, const char *file, int line, const char *fmt, va_list args )
{
	...
	{
		/** 
			20170418. Si la funci�n genera una excepci�n, no puedo volver a llamarla porque podr�a dar lugar a un bucle infinito.
		 */
