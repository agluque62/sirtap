...
// #include "Severity.h"
	...
	public:
		...
		/****/
		/* ReConfigure() no permite cambiar los valores de filename ni el udpID ya que en el arranque se personalizan con el tipo de modulo
		 * y si es un recurso, con el numero del recurso */
		void ReConfigure()
		{
		...
		void TestCfgChange()
		{
			...
			lstat( _fileConfig.c_str(), &_currentMod );
			if (_lastMod.st_mtime != _currentMod.st_mtime)
			{
				_lastMod.st_mtime = _currentMod.st_mtime;
				change = true;
...
//cambio LOG por LOG1 porque la libreria snmp tiene otro define LOG
//#define LOG(severity)                   LOG_(0, severity)
#define LOG1(severity)                   LOG_(0, severity)

#define LOG_VERBOSE                     LOG1(plog::verbose)
#define LOG_DEBUG                       LOG1(plog::debug)
#define LOG_INFO                        LOG1(plog::info)
#define LOG_WARNING                     LOG1(plog::warning)
#define LOG_ERROR                       LOG1(plog::error)
#define LOG_FATAL                       LOG1(plog::fatal)
