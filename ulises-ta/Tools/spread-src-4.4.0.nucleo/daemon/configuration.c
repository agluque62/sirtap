...
void	Conf_load_conf_file( char *file_name, char *my_name )
{
	...
	if (NULL != (yyin = fopen(file_name,"r")) )
                Alarm( DEBUG /* AGL PRINT*/, "Conf_load_conf_file: using file: %s\n", file_name);
	if (yyin == NULL) 
		if (NULL != (yyin = fopen("./spread.conf", "r")) )
                        Alarm( DEBUG /* AGL PRINT*/, "Conf_load_conf_file: using file: ./spread.conf\n");
	if (yyin == NULL)
...
{
        if (new_state == FALSE) {
                Alarm(DEBUG/*AGL PRINT*/, "disabling Dangerous Monitor Commands!\n");
        } else if (new_state == TRUE) {
                Alarm(DEBUG/*AGL PRINT*/, "ENABLING Dangerous Monitor Commands! Make sure Spread network is secured\n");
        } else {
                /* invalid setting */
                return;
        }
...
static void set_param_if_valid(char **param, char *value, char *description, unsigned int max_value_len)
{
        if (value != NULL && *value != '\0')
        {
                ...
                *param = buf;
                ...
                Alarm(DEBUG /* AGL PRINT*/, "Set %s to '%s'\n", description, value);
        }
        else
        {
