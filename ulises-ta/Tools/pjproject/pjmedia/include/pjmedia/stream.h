...
typedef enum Stream_CallFlags
{
	STREAM_RXONLY = 0x1,			//The stream only transmits
	STREAM_TXONLY = 0x2,			//The stream only receives
	STREAM_IDLE = 0x4,				//The stream not transmits nor receives
	STREAM_PTTID_ENABLED = 0x8		//there is  not PTT-id. Then PTT ON also cannot be sent
} Stream_CallFlags;
...
extern unsigned gJBufPframes;

/* Parametros que definen el buffer de jitter del agente*/
extern unsigned pjmedia_stream_jitter_buffer_force;				//Fuerza los parametros del buffer de jitter
extern unsigned pjmedia_stream_jitter_buffer_max_count;			//Capacidad maxima del buffer de Jitter
extern unsigned pjmedia_stream_jitter_buffer_adaptive;			//Si es distinto de cero, entonces el buffer es adaptativo, si es 0 entonces es fijo
extern unsigned pjmedia_stream_jitter_buffer_prefetch;			//Prefetch del buffer de jitter.
...
extern unsigned pjmedia_stream_jitter_buffer_min_prefetch;		//Para buffer adaptativo: The minimum delay that must be applied to each incoming packets, in number of frames.
extern unsigned pjmedia_stream_jitter_buffer_max_prefetch;		//Para buffer adaptativo: The maximum allowable value for prefetch delay, in number of frames.
extern unsigned pjmedia_stream_jitter_buffer_discard;			//Para buffer adaptativo(en fijo se ignora) :
...
PJ_DECL(void) 
pjmedia_stream_set_rtp_ext_tx_info(pjmedia_stream * stream, pj_uint32_t rtp_ext_tx_info);

PJ_DECL(void) 
pjmedia_stream_get_rtp_ext_tx_info(pjmedia_stream * stream, pj_uint32_t *rtp_ext_tx_info);

/* NUCLEO 2014 */
/*
PJ_DECL(void) 
pjmedia_stream_enable_ec(pj_pool_t *pool, pjmedia_stream * stream);
*/
PJ_DECL(void) pjmedia_stream_enable_ec(pj_pool_t *pool, pjmedia_stream * stream, unsigned tail_ms, unsigned latency_ms, unsigned options);
/* FM */

PJ_DECL(unsigned long long) pjmedia_stream_GetTimeClimax(void);
PJ_DECL(void) pjmedia_stream_set_climax_param(pjmedia_stream * stream, pj_bool_t NTP_synchronized);
PJ_DECL(void) pjmedia_stream_set_request_MAM(pjmedia_stream * stream);
PJ_DECL(void) pjmedia_stream_send_MAM(pjmedia_stream* stream, pj_uint32_t TQV, pj_uint32_t T1, pj_uint32_t T2, pj_uint16_t Tj1, pj_uint16_t Tid, pj_uint16_t Ts2);
PJ_DECL(void) pjmedia_stream_force_send_KA_packet(pjmedia_stream * stream);
PJ_DECL(void) pjmedia_stream_force_set_impairments(pj_pool_t* pool, pjmedia_stream * stream, int Perdidos, int Duplicados, int LatMin, int LatMax);
PJ_DECL(void) pjmedia_stream_reset_ext_header(pjmedia_stream * stream);
...
PJ_DECL(void) pjmedia_stream_GRS_force_ptt_in_RTPRx(pjmedia_stream* stream, pj_uint32_t PttType, pj_uint32_t PttId);
PJ_DECL(void) pjmedia_stream_GRS_force_SCT_in_RTPRx(pjmedia_stream* stream, pj_bool_t on);
PJ_DECL(void) pjmedia_stream_force_PTTS_in_RTPTxRx(pjmedia_stream* stream, pj_bool_t on);
PJ_DECL(pj_bool_t) pjmedia_stream_get_GRS_force_ptt_in_RTPRx_confirmed(pjmedia_stream* stream);
...
*
* @return		User data of the stream.
*/
PJ_DECL(void*) pjmedia_stream_get_user_data( pjmedia_stream *stream);
