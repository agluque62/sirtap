...
PJ_DEF(pj_status_t) pjmedia_sdp_neg_get_initial_sdp(pjmedia_sdp_neg* neg,
	const pjmedia_sdp_session** initial_sdp)
{
	PJ_ASSERT_RETURN(neg && initial_sdp, PJ_EINVAL);

	*initial_sdp = neg->initial_sdp;
	return PJ_SUCCESS;
}
			       ...
			       pjmedia_sdp_media **p_answer)
{
	...
	unsigned index_in_slave_than_codec_123_is = 0;
	pj_bool_t found_codec_123_in_master = PJ_FALSE;
	pj_bool_t found_codec_123_in_slave = PJ_FALSE;
	...
	/*Buscamos el codec 123 en el master, y averiguamos si no es el unico codec. 
	Si es el unico entonces lo dejariamos seleccionado. El seleccionado quedaria el primero de la lista
	Si no es el unico seleccionariamos el primero que aparezca que no es el 123*/
	for (i = 0; i < master->desc.fmt_count; ++i) {
		if (pj_isdigit(*master->desc.fmt[i].ptr)) {			
			...
			if (pt == 123)
			{
				found_codec_123_in_master = PJ_TRUE;				
				break;
			}
		}
	}

	//Lo buscamos en el slave
	{
		unsigned j;
		for (j = 0; j < slave->desc.fmt_count; ++j) {
			...
			if (p == 123) {
				found_codec_123_in_slave = PJ_TRUE;
				index_in_slave_than_codec_123_is = j;
				break;
			}
		}
	}
	...
	if (!found_codec_123_in_master || !found_codec_123_in_slave)
	{
		//Si alguno de los dos no tiene el 123, tienen que coindicir alguno de los otros codecs
	...
	//En este punto, o los dos tienen el 123, o alguno de los dos no lo tiene, pero otro codec coincide

	if (found_codec_123_in_slave)
	{
		//Si el slave (el que responde) tiene el codec 123, lo ponemos al final si o si. Es obligado por la ED137
		pt_answer[pt_answer_count++] = slave->desc.fmt[index_in_slave_than_codec_123_is];
	}
