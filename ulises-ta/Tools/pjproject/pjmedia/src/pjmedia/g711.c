/* $Id: g711.c 2760 2009-06-13 15:28:37Z nanang $ */
/* 
...
static pj_status_t  g711_decode( pjmedia_codec *codec, 
				 const struct pjmedia_frame *input,
				 unsigned output_buf_len, pj_bool_t reg_in_plc, 
				 struct pjmedia_frame *output);
#if !PLC_DISABLED
				   ...
				   const pjmedia_codec_info *id )
{
    ...
    return (id->pt==PJMEDIA_RTP_PT_PCMU || id->pt==PJMEDIA_RTP_PT_PCMA || id->pt == PJMEDIA_RTP_PT_R2S)? 0:-1;
}
				    ...
				    pjmedia_codec_info codecs[])
{
	...
	if (count < *max_count) {
		codecs[count].type = PJMEDIA_TYPE_AUDIO;
		codecs[count].pt = PJMEDIA_RTP_PT_R2S;
		codecs[count].encoding_name = pj_str("R2S");
		codecs[count].clock_rate = 8000;
		codecs[count].channel_cnt = 1;
		++count;
	}
			        ...
			        const pjmedia_codec_param *attr )
{
    ...
    if (attr->info.pt == PJMEDIA_RTP_PT_R2S || priv->pt == PJMEDIA_RTP_PT_R2S)
    {
        priv->pt = attr->info.pt;
    }
    else if (attr->info.pt != priv->pt)
    {
				...
				struct pjmedia_frame *output)
{
    ...
    /* Encode */
    if (priv->pt == PJMEDIA_RTP_PT_PCMA /* || priv->pt == PJMEDIA_RTP_PT_R2S*/) {
	unsigned i, n;
...
#if !PLC_DISABLED
    if (priv->plc_enabled && reg_in_plc)
