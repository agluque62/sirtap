/* $Id: pjsua_app.c 3128 2010-03-30 11:13:59Z nanang $ */
/* 
			     ...
			     pjsua_call_id replaced_call_id, pjsip_rx_data *rdata)
{
	...
	PJ_UNUSED_ARG(replaced_call_id);
    PJ_UNUSED_ARG(rdata);
...
void console_app_main(const pj_str_t *uri_to_call)
{
	...
	case 'v':
	     ...
	     */
	    if (current_call != -1) {
				/** AGL 140528 Añadir el último parámetro.... */
			pjsua_call_reinvite(current_call, PJ_TRUE, NULL, NULL);
...
pj_status_t app_main(void)
{
	...
	pj_pool_t * _Pool;
	//pjmedia_port _Port;
	pjmedia_port_info _info;
	pjmedia_snd_port *_SndPorts[5];
	pj_str_t name1 = pj_str("PSVP");
	...
	/** AGL. Pruebas de Sound Port...*/
	_Pool = pjsua_pool_create(NULL, 4096, 512);

	status = pjmedia_port_info_init(&_info, 
		&name1, 
		PJMEDIA_PORT_SIGNATURE('P', 'S', 'V', 'P'), 
		SAMPLING_RATE, 
		CHANNEL_COUNT, 
		BITS_PER_SAMPLE, 
		SAMPLES_PER_FRAME);

	status = pjmedia_snd_port_create_player(_Pool, 0, SAMPLING_RATE, CHANNEL_COUNT, 
				((SAMPLING_RATE * CHANNEL_COUNT * PTIME) / 1000), BITS_PER_SAMPLE, 0, _SndPorts);
