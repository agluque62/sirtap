...
/*En este puerto se recibe el audio procedente del NBOX por multicast.
Se crea un socket para recibir el audio. Cada vez que se recibe audio se almacena en un buffer 
jitter _Jbuf, y en la funcion GetFrame se extraen los datos de ese _Jbuf para el puerto.*/
							...
							const char * localIp, const char * mcastIp, unsigned mcastPort)
{
	...
	try
	{
		...
		/*_Sock. Socket por el que se recibe el audio distribuido por la dirección multicast*/
		...
		PJ_LOG(5,(__FILE__, "BSS: RdRxPort::RdRxPort McastAddr %s Port %d ", localIp, mcastPort));
...
pj_status_t RdRxPort::GetFrame(pjmedia_port * port, pjmedia_frame * frame)
{
	...
	pj_int16_t * p_out_samp = (pj_int16_t*)frame->buf;							// Las tramas recibidas (en el MCAST) son 16 bit con signo
	...
	{
		char frame_type;
		pj_size_t frame_size = samples_per_frame * 2;
		...
		if (frame_type == PJMEDIA_JB_MISSING_FRAME) 
		{
			...
			PJ_LOG(5, (port->info.name.ptr, "WARNING: FALTA LOST PJMEDIA_JB_MISSING_FRAME port %p", port));
		...
		else if (frame_type == PJMEDIA_JB_ZERO_EMPTY_FRAME) 
		{
			...
			* the frame. 
			*/
			//PJ_LOG(5, (port->info.name.ptr, "TRAZA: FALTA LOST PJMEDIA_JB_ZERO_EMPTY_FRAME port %p", port));
		...
		else if (frame_type != PJMEDIA_JB_NORMAL_FRAME) 
		{
			...
			PJ_LOG(5, (port->info.name.ptr, "WARNING: FALTA LOST PJMEDIA_JB_ZERO_PREFETCH_FRAME"));
...
{
	/*Aquí se reciben los datos de _Socket y se guardan en el buffer jitter _Jbuf, de donde luego se extraen los datos en GetFrame*/

	RdRxPort * pThis = (RdRxPort*)pj_activesock_get_user_data(asock);
	if (!pThis) return PJ_TRUE;
	...
	if (status == PJ_SUCCESS)
	{
		if (size == ((pThis->_RemoteSamplesPerFrame * (BITS_PER_SAMPLE / 8)) + sizeof(unsigned)))
		{
			...
			//pjmedia_jbuf_put_frame2(pThis->_Jbuf, data, audio_size, 0, (int)pj_ntohl(ext_seq), NULL);
		}
		else if (size == 1)
		{
