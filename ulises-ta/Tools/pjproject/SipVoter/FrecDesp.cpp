 ...
 */
int FrecDesp::AddToGroup(char *rdfr, pjsua_call_id call_id, SipCall *psipcall, char *Bss_selected_method, CORESIP_CallFlagsMask flags)
{
		...
		if (ret == 0)
		{
			for (j = 0; j < MAX_SESSIONS; j++)
			{
				if (groups[i].sessions[j].sess_callid == PJSUA_INVALID_ID)
				{
					...
					groups[i].sessions[j].CallFlags = flags;
...
int FrecDesp::RemFromGroup(int index_group, int index_sess)
{
	...
	if ((groups[index_group].sessions[index_sess].CallFlags & CORESIP_CALL_RD_RXONLY) && (groups[index_group].nsessions_rx_only > 0))
		groups[index_group].nsessions_rx_only--;
	else if ((groups[index_group].sessions[index_sess].CallFlags & CORESIP_CALL_RD_TXONLY) && (groups[index_group].nsessions_tx_only > 0))
...
/**
 * CalcTimeDelay.	...
 * Calcula los tiempos de retardo a partir de lo recibido en la extesion de cabecera
 * @param stream. Es el stream en el que se ha recibido el TLV
 * @param   ext_value		Puntero donde comienza el value del TLV
 * @param					Retorna estos valores en ms Tn1_ms, Tj1_ms, Tid_ms, Tsd_ms
 * @param					Retorna estos valores en unidades de 125us Tn1, Tj1, Tid, Tsd
 * @param	requested_climax_method. Metodo en el que se calcula el retardo
 * @param	used_climax_method. retorna el metodo usado realmente.
...
{
	pj_uint32_t TQG, T1, NMR, T2, T4;

	pj_assert(length == 12 || length == 14);		//En ED137B length es 12 y en ED137C es 14

	*request_MAM = PJ_FALSE;
	...
	Tsd_ms = (unsigned int)Tsd;		//En unidades de 125 us
	Tsd_ms /= 8;						// Tn1 * 125 / 1000 = Tn1 / 8  (ms)
	...
	Tj1_ms = (unsigned int)Tj1;		//En unidades de 125 us
	Tj1_ms /= 8;						// Tn1 * 125 / 1000 = Tn1 / 8  (ms)
	...
	Tid_ms = (unsigned int)Tid;		//En unidades de 125 us
	Tid_ms /= 8;						// Tn1 * 125 / 1000 = Tn1 / 8  (ms)

	if (length == 14)
	{
		ext_value++;
		Ts2 = (pj_uint32_t)*ext_value;
		Ts2 <<= 8;
		ext_value++;
		Ts2 |= (pj_uint32_t)*ext_value;

		Ts2_ms = (unsigned int)Ts2;		//En unidades de 125 us
		Ts2_ms /= 8;						// Tn1 * 125 / 1000 = Tn1 / 8  (ms)
	}
	else
	{
		Ts2 = 0;
	...
	if (requested_climax_method == Absolute && TQG != 0)
	{
		if (T2 >= T1)
		{
			//Metodo absoluto
	...
	CORESIP_CLD_CALCULATE_METHOD used_climax_method)
{
	...
	groups[index_group].sessions[index_sess].Tsd = Tsd;	
	...
	else groups[index_group].sessions[index_sess].cld_absoluto = PJ_FALSE;
...
/**
 * GetRMMData.	...
 * Obtiene los parametros recibidos del RMM recibido.
 * @param   ext_value		Puntero donde comienza el value del TLV
 * @param	TQV. Valor devuelto de TQV
 * @param	T1. Valor devuelto de T1
 * @return	-1 su hay error.
 */
int FrecDesp::GetRMMData(pj_uint8_t* ext_value, pj_uint32_t *TQV, pj_uint32_t *T1)
{
	*TQV = (pj_uint32_t)*ext_value;
	*TQV >>= 7;
	*TQV &= 0x1;
	...
	for (i = 0; i < MAX_GROUPS; i++)
	{
		if (strlen(groups[i].RdFr) > 0 && strlen(groups[i].Zona) > 0)
		{
			...
			for (j = 0; j < MAX_SESSIONS; j++)
			{
				...
				if (!(groups[i].sessions[j].CallFlags & CORESIP_CALL_RD_RXONLY))
				{
...
int FrecDesp::ClrAllSquSt(int index_group)
{
	...
	if (strlen(groups[index_group].RdFr) > 0 && strlen(groups[index_group].Zona) > 0)
	{
		...
		{				
			...
			if (groups[index_group].sessions[j].pSipcall != NULL)
			{
				groups[index_group].sessions[j].pSipcall->last_received_squ_status = PJ_FALSE;
			}				
		}
	}
	else
 ...
 * @param	index_group		Indice del grupo
 * @param   sipcall_in_window_timer. Si es distinto de NULL es la llamada que arranca la ventana de decision, si es NULL, entonces no hay ventana decision en curso
 * @return	Retorna -1 si no existe el grupo. 
 */
int FrecDesp::SetInWindow(int index_group, SipCall *sipcall_in_window_timer)
{
	int ret = 0;
	...
	if (strlen(groups[index_group].RdFr) > 0 && strlen(groups[index_group].Zona) > 0)
	{
		groups[index_group].sipcall_in_window_timer = sipcall_in_window_timer;
	}
	else
	{
		ret = -1;
 ...
 * Activa en todas las sesiones del grupo el flag que indica el estado de la ventana de seleccion bss 
 * @param	index_group		Indice del grupo
 * @return	Si es distinto de NULL es la llamada que arranca la ventana de decision, si es NULL, entonces no hay ventana decision en curso 
 */
SipCall* FrecDesp::GetInWindow(int index_group)
{
	SipCall* ret = NULL;
	...
	if (strlen(groups[index_group].RdFr) > 0 && strlen(groups[index_group].Zona) > 0)
	{
		ret = groups[index_group].sipcall_in_window_timer;
	}
	else
	{
		ret = NULL;
...
/**
 * SetQidxMethod.	...
 * Asigna el metodo del Qidx a la sesion correspondiente.
 * @param	index_group		Indice del grupo
 * @param   index_sess		Indice de la sesion
 * @param   qidx_method		Metodo de calculo
 * @return	-1 su hay error.
 */
int FrecDesp::SetQidxMethod(int index_group, int index_sess, pj_uint8_t qidx_method)
{
	if (index_group < 0 || index_sess < 0 || index_group >= MAX_GROUPS || index_sess >= MAX_SESSIONS) 
	{
		return -1;
...
/**
 * SetAirQidxInWindows.	...
 * Asigna AirQidxInWindows. Valor del Qidx al final de la ventana de decision cuando el squelch no es de avion, que sirve para crear la lista de Qidx del grupo
 * @param	index_group		Indice del grupo
 * @param   index_sess		Indice de la sesion
 * @return	-1 su hay error.
 */
int FrecDesp::SetAirQidxInWindows(int index_group, int index_sess)
{
	if (index_group < 0 || index_sess < 0 || index_group >= MAX_GROUPS || index_sess >= MAX_SESSIONS)
	{
		return -1;
	...
	if (strlen(groups[index_group].RdFr) > 0 && strlen(groups[index_group].Zona) > 0)
	{
		...
		groups[index_group].sessions[index_sess].AirQidxInWindows = groups[index_group].sessions[index_sess].pSipcall->GetSyncBss();
	}
	pj_mutex_unlock(fd_mutex);
 ...
 * @return	el valor.
 */
int FrecDesp::GetAirQidxInWindows(int index_group, int index_sess)
{
	if (index_group < 0 || index_sess < 0 || index_group >= MAX_GROUPS || index_sess >= MAX_SESSIONS)
	{
		return 0;
	...
	if (strlen(groups[index_group].RdFr) > 0 && strlen(groups[index_group].Zona) > 0)
	{
		...
		ret = groups[index_group].sessions[index_sess].AirQidxInWindows;
	}
	pj_mutex_unlock(fd_mutex);
...
/**
* ClearBss_bss_rx_In_Group
* reinicia los arrays bss_rx del grupo. 
* @param	index_group		Indice del grupo
* @return	-1 su hay error.
*/
void FrecDesp::Clear_SyncBss_In_Group(int index_group)
{
	if (index_group < 0 || index_group >= MAX_GROUPS)
	{
		return;
	...
	for (int j = 0; j < MAX_SESSIONS; j++)
	{
		...
		{
			groups[index_group].sessions[j].pSipcall->InitSyncBss();
		}
	}
	pj_mutex_unlock(fd_mutex);
}
...
* @return	-1 su hay error.
*/
int FrecDesp::GetClimaxMethod(int index_group, int index_sess, CORESIP_CLD_CALCULATE_METHOD &climax_method)
{
	if (index_group < 0 || index_sess < 0 || index_group >= MAX_GROUPS || index_sess >= MAX_SESSIONS)
	{
		return -1;
	...
	climax_method = groups[index_group].sessions[index_sess]._MetodoClimax;
...
int FrecDesp::SetBetterSession(SipCall *p_current_sipcall, pj_bool_t in_window, pj_bool_t only_selected_in_window)
{
	...
	{
		PJ_LOG(5,(__FILE__, "BSS: SetBetterSession current session  FR %s %s qidx %d", groups[index_group].RdFr, 
	...
	{						
		if (groups[index_group].sessions[j].sess_callid != PJSUA_INVALID_ID &&
			...
			groups[index_group].sessions[j].pSipcall != NULL)
		{
			unsigned int call_flags = groups[index_group].sessions[j].CallFlags;
			...
			if (!(call_flags & CORESIP_CALL_RD_TXONLY))
			{
				if (j != index_sess)
				{
					...
					if (SipAgent::Coresip_Local_Config._Debug_BSS)
					{
						PJ_LOG(5,(__FILE__, "BSS: SetBetterSession FR %s %s qidx %d", groups[index_group].RdFr, 
							groups[index_group].sessions[j].pSipcall->DstUri, bss_sync));
					}

					if ((stricmp(groups[index_group].sessions[j].Bss_selected_method, curBssSelMethod) == 0) &&
						(bss_sync > better_bss))
					{
						better_index_sess = j;
						better_bss = bss_sync;
				 ...
				 (strcmp(groups[index_group].SelectedUri, groups[index_group].sessions[better_index_sess].pSipcall->DstUri) == 0)))
			{
					...
					SipAgent::Coresip_Local_Config._Test_ED137C_Dynamic_Delay_Compensation)
				{
					PJ_LOG(3,(__FILE__, "BSS: ID DESTINO %s FR %s %s SELECCIONADO", 
						...
						groups[index_group].sessions[better_index_sess].pSipcall->_RdFr,
			...
			if (!(call_flags & CORESIP_CALL_RD_TXONLY))
			{
				...
				if (ses != NULL)
				{
					...
					if (call)
					{
						...
						groups[index_group].sessions[j].pSipcall->RdInfo_prev.Squelch = groups[index_group].sessions[j].pSipcall->last_received_squ_status;
						...
						if (groups[index_group].sessions[j].pSipcall->Ptt_off_timer.id == 0)
						{
							...
							//-->
							if (groups[index_group].sessions[j].pSipcall->last_received_squ_status == 0)
							{
								info_aux.rx_qidx = 0;
							}
...
SipCall* FrecDesp::GetSelected(int index_group)
{
	if (index_group < 0 || index_group >= MAX_GROUPS)
	{
		return NULL;
	...
	for (int j = 0; (j < MAX_SESSIONS); j++)
	{
		...
		if (groups[index_group].sessions[j].selected) return groups[index_group].sessions[j].pSipcall;	
	}
	pj_mutex_unlock(fd_mutex);
	...
	while (wp->ntp_check_thread_run)
	{
		if (wp->ngroups == 0)
		{
			...
			for (unsigned int i = 0; i < ((PERIOD_CHECK_NTP*multiplier)/mysleep); i++)
			{
				...
				if (!wp->ntp_check_thread_run) break;
			}
			continue;
		}
