...
pj_status_t SoundRxPort::GetFrame(pjmedia_port * port, pjmedia_frame * frame)
{
	...
	pjmedia_delay_buf* delay_buf = pThis->_SndInBufs[port->port_data.ldata];

	//Si el delay buffer no tiene suficientes muestras entonces nos salimos de esta funcion
	//Si no lo hicieramos, entonces la funcion pjmedia_delay_buf_get se inventa muestras y esto se ha visto que
	//es perjudicial ya que eso ya se hace al enviar por RTP en el fichero stream.c, funcion put_frame_imp 	
	/*if (pjmedia_delay_buf_need_samples(delay_buf))
	{
		return 1;
	}	
	*/
	...
	return pjmedia_delay_buf_get(delay_buf, (pj_int16_t*)frame->buf);
}
