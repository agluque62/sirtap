/* $Id: sip_inv.c 2697 2009-05-13 08:57:38Z bennylp $ */
/* 
...
static pj_status_t inv_send_ack(pjsip_inv_session *inv, pjsip_event *e)
{
	...
	status = (*mod_inv.cb.on_send_ack)(inv, rdata, e);
	if (status == PJ_SUCCESS)
	{
					  ...
					  pjsip_inv_session **p_inv)
{
    ...
    *p_inv = inv;
    ...
    PJ_LOG(5,(inv->obj_name, "UAC invite session created for dialog %s", dlg->obj_name));
					      ...
					      pjsip_tx_data **p_tdata)
{
	...
	*options |= PJSIP_INV_SUPPORT_TIMER;
	...
	if (l_sdp) {
		...
		/* Incompatible media */
		/** 2016062. AGL. Codigo Requerido por VOTER */
		code = PJSIP_SC_UNSUPPORTED_MEDIA_TYPE;
...
PJ_DEF(pj_bool_t) pjsip_inv__is_re_invite_radio_type(pjsip_inv_session* inv)
{
	//Retorna true si es un re-invite de una sesion radio
	pjmedia_sdp_session* active_local_sdp;
	pjmedia_sdp_session* active_remote_sdp;
	pjmedia_sdp_attr* attr;
	pj_status_t st;
	unsigned int i;
	pj_bool_t is_local_type_radio = PJ_FALSE;
	pj_bool_t is_remote_type_radio = PJ_FALSE;

	st = pjmedia_sdp_neg_get_active_local(inv->neg, &active_local_sdp);
	if (st == PJ_SUCCESS && inv->state == PJSIP_INV_STATE_CONFIRMED)
	{
		//Nos encontramos en un re-invite. Analizamos si el tipo de sesion es de Radio

		st = pjmedia_sdp_neg_get_active_remote(inv->neg, &active_remote_sdp);
		if (st == PJ_SUCCESS)
		{
			for (i = 0; i < active_local_sdp->media_count; i++)
			{
				active_local_sdp->media[i];
				attr = pjmedia_sdp_attr_find2(active_local_sdp->media[i]->attr_count, active_local_sdp->media[i]->attr, "type", NULL);
				if (attr != NULL)
				{
					...
					{
						is_local_type_radio = PJ_TRUE;
						break;
					}
				}
			}

			for (i = 0; i < active_remote_sdp->media_count; i++)
			{
				active_remote_sdp->media[i];
				attr = pjmedia_sdp_attr_find2(active_remote_sdp->media[i]->attr_count, active_remote_sdp->media[i]->attr, "type", NULL);
				if (attr != NULL)
				{
					if ((pj_strnicmp2(&attr->value, "Radio", 5) == 0) ||
						(pj_strnicmp2(&attr->value, "Coupling", 8) == 0) ||
	...
	if (is_local_type_radio == PJ_TRUE && is_remote_type_radio == PJ_TRUE)
	{
		return PJ_TRUE;
	}
	else
...
static pj_status_t inv_negotiate_sdp( pjsip_inv_session *inv )
{
	...
	if (pjsip_inv__is_re_invite_radio_type(inv))
	{
		return status;
	}
					    ...
					    pjsip_tx_data **p_tdata )
{
	...
	if (inv->role == PJSIP_ROLE_UAC) {
		...
		*p_tdata = NULL;
	     ...
	     */
	    status = pjsip_tsx_set_timeout(inv->invite_tsx, pjsip_cfg()->tsx.tsx_tout);
		if (status != PJ_SUCCESS)
		{
			return status;
		}
					...
					pjsip_event *e)
{
	...
	/* Pongo la siguiente linea para que cuando se reciba un CANCEL poder capturar parametros
	del mensaje en la on_tsx_state_changed. Si no hago esto entonces la pjsip solo nofica 
	el envio del 487 */
					 ...
					 pjsip_event *e)
{
    ...
    if (inv->state != PJSIP_INV_STATE_DISCONNECTED &&
		(tsx->method.id == PJSIP_OTHER_METHOD && pj_strcmp2(&(tsx->method.name), "INFO") != 0) &&
		(tsx->method.id == PJSIP_OTHER_METHOD && pj_strcmp2(&(tsx->method.name), "NOTIFY") != 0) &&
...
static void inv_on_state_incoming( pjsip_inv_session *inv, pjsip_event *e)
{
	...
	inv_respond_incoming_cancel(inv, tsx, e->body.tsx_state.src.rdata, e);
...
static void inv_on_state_early( pjsip_inv_session *inv, pjsip_event *e)
{
	 ...
	 * for INVITE.
	 */
	pjsip_tsx_set_timeout(inv->invite_tsx, pjsip_cfg()->tsx.tsx_tout);
    }
}
		...
		*/
		if (tsx->state == PJSIP_TSX_STATE_TRYING) {
			...
			if (status != PJ_SUCCESS) {
				...
				status = pjsip_dlg_create_response(inv->dlg, rdata, 488, NULL, &tdata);
				if (status != PJ_SUCCESS)
					return;


				accept = pjsip_endpt_get_capability(dlg->endpt, PJSIP_H_ACCEPT,	NULL);
				if (accept) {
			...
			*/
			if (rdata->msg_info.msg->body != NULL) {
				...
				int on_reinvite_returned_code = PJSIP_SC_OK;
				pj_str_t* on_reinvite_returned_cause = NULL;

				if (mod_inv.cb.on_reinvite_received)
				{				
					(*mod_inv.cb.on_reinvite_received)(inv, e, tdata, &on_reinvite_returned_code, &on_reinvite_returned_cause);

					if (on_reinvite_returned_code != PJSIP_SC_OK)
					{
						status = pjsip_dlg_create_response(dlg, rdata, on_reinvite_returned_code, NULL, &tdata);
						if (status == PJ_SUCCESS)
						{
							if (on_reinvite_returned_cause != NULL)
							{
								const pj_str_t reason_name = pj_str("Reason");

								pjsip_generic_string_hdr* reason;
								reason = pjsip_generic_string_hdr_create(tdata->pool, &reason_name, on_reinvite_returned_cause);
								if (reason) pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*)reason);
							}
				...
				if (mod_inv.cb.on_create_offer)  {
					...
					if (sdp) {
						...
						*/
						status = pjmedia_sdp_neg_modify_local_offer(inv->pool_prov, inv->neg, sdp);
				...
				if (sdp == NULL) {
					...
					status = pjmedia_sdp_neg_send_local_offer(inv->pool_prov, inv->neg, &active_sdp);
					if (status == PJ_SUCCESS)
