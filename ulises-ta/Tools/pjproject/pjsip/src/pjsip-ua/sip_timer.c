...
void timer_cb(pj_timer_heap_t *timer_heap, struct pj_timer_entry *entry)
{
    ...
    if (as_refresher) 
	{
		...
		else 
		{
			...
			{
				//Se agregan las cabeceras subject y priority si se hizo en el invite inicial
				if (inv->subject.hvalue.slen > 0 && inv->priority.hvalue.slen > 0)
				{
					pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*) &inv->subject);
					pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*) &inv->priority);
				}
			}
...
PJ_DEF(pj_status_t) pjsip_timer_init_module(pjsip_endpoint *endpt)
{
	...
	/** AGL... QUITO ESTO PARA NO DAR SOPORTE A LOS TIMERS ??? 
	...
	*/
    ...
    status = pjsip_register_hdr_parser( STR_SE.ptr, STR_SHORT_SE.ptr, &parse_hdr_se);
    if (status != PJ_SUCCESS)
		return status;
    ...
    status = pjsip_endpt_add_capability(endpt, NULL, PJSIP_H_SUPPORTED, NULL, 1, &STR_TIMER);
    if (status != PJ_SUCCESS)
		return status;
					     ...
					     pjsip_status_code *st_code)
{
    ...
    PJ_ASSERT_ON_FAIL(inv && rdata, {if(st_code)*st_code=PJSIP_SC_INTERNAL_SERVER_ERROR;return PJ_EINVAL;});
	...
	else if (msg->line.status.code/100 == 2) 
	{
		...
		/** AGL...*/
		// PJ_LOG(4, ("AGL1","Arrancado TIMER SESSION %d-%d", inv->timer->setting.min_se,inv->timer->setting.sess_expires));
	}
