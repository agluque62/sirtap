/* $Id: evsub.c 3068 2010-01-21 10:04:26Z bennylp $ */
/* 
...
struct pjsip_evsub
{
	...
	int maximum_expires_as_uas;			/**< Valor maximo de expires soportado como UAS. Si es negativo se ignora	    */
	int minimum_expires_as_uas;			/**< Valor minimo de expires soportado como UAS. Si es negativo se ignora	    */
...
/*
 * Get event subscription's dlg.
 */
PJ_DEF(pjsip_dialog*) pjsip_evsub_get_dlg( pjsip_evsub *sub )
{    
	return sub->dlg;
}
...
/*
 * UnRegister an event package
 */
PJ_DEF(pj_status_t) pjsip_evsub_unregister_pkg(pjsip_module* pkg_mod,
	const pj_str_t* event_name)
{
	struct evpkg* pkg;
	...
	pkg = find_pkg(event_name);
	if (pkg == NULL)
	{
		return PJ_EINVALIDOP;
	}

	if (pj_list_find_node(&mod_evsub.pkg_list, pkg) == NULL)
		return PJ_EINVALIDOP;
...
static void evsub_destroy( pjsip_evsub *sub )
{
    ...
    PJ_LOG(3, (sub->obj_name, "INFO: Subscription destroyed event type %.*s", sub->event->event_type.slen, sub->event->event_type.ptr));
...
/*
 *  search_subscription_in_dialog
 * Busca una subscripcion en un dialogo sip
 * @param dlg. Dialogo
 * @param event_type. Tipo de evento de la subscripcion
 * Return. Puntero a la subscripcion. NULL si no hay ninguna.
 */
PJ_DEF(pjsip_evsub*) search_subscription_in_dialog(pjsip_dialog* dlg, pj_str_t* event_type)
{
	struct dlgsub* dlgsub_head = NULL, * dlgsub = NULL;
	pjsip_evsub* ret_sub = NULL;
	pj_bool_t sub_in_dialog = PJ_FALSE;
	...
	if (dlgsub_head != NULL)
	{
		...
		while (dlgsub != dlgsub_head)
		{
			if (pj_stricmp(&dlgsub->sub->event->event_type, event_type) == 0)
			{
				sub_in_dialog = PJ_TRUE;
				break;
			}
			dlgsub = dlgsub->next;
		}
	...
	if (sub_in_dialog == PJ_TRUE)
	{
		if (dlgsub != NULL) ret_sub = dlgsub->sub;
	}
				 ...
				 pjsip_evsub **p_evsub )
{
	...
	sub->maximum_expires_as_uas = -1;
	sub->minimum_expires_as_uas = -1;
 ...
 */
PJ_DEF(pj_int32_t) pjsip_evsub_get_expires(pjsip_evsub* sub)
{
 ...
 */
PJ_DEF(void) pjsip_evsub_set_state(pjsip_evsub *sub, pjsip_evsub_state state)
{
    sub->state = state;
	sub->state_str = evsub_state_names[sub->state];
}
...
/*
 * Set subscription user data.
 */
PJ_DEF(void) pjsip_evsub_set_user_data(pjsip_evsub *sub, void *user_data)
{
	sub->user_data = user_data;
}

/*
 * Set maximum_expires_as_uas
 */
PJ_DEF(void) pjsip_evsub_set_maximum_expires_as_uas(pjsip_evsub* sub, int maximum_expires_as_uas)
{
	sub->maximum_expires_as_uas = maximum_expires_as_uas;
}

/*
 * Set minimum_expires_as_uas
 */
PJ_DEF(void) pjsip_evsub_set_minimum_expires_as_uas(pjsip_evsub* sub, int minimum_expires_as_uas)
{
	sub->minimum_expires_as_uas = minimum_expires_as_uas;
}

/*
 * Get subscription user data.
 */
PJ_DEF(void *) pjsip_evsub_get_user_data(pjsip_evsub *sub)
{
					...
					const pjsip_hdr *hdr_list )
{
    ...
    /* Add expires header: */
	if (sub->maximum_expires_as_uas > 0 && sub->maximum_expires_as_uas < sub->expires->ivalue)
	{
		sub->expires->ivalue = sub->maximum_expires_as_uas;
	}	
 ...
 */
PJ_DEF(pj_status_t) pjsip_evsub_reject(pjsip_evsub* sub,
	pjsip_rx_data* rdata,
	int st_code,
	const pjsip_hdr* hdr_list)
{
...
/*
 * Create a forced Subscription-State header based on parameters
 */
static void modify_sub_state_hdr(pj_pool_t* pool,
								pjsip_sub_state_hdr* sub_state,
								const int expires,			//Value -1 if it is not required to be modified
								const pj_str_t* state_str,	//Value NULL if it is not required to be modified
								const pj_str_t* reason,		//Value NULL if it is not required to be modified
								const int retry_after)		//Value -1 if it is not required to be modified
{
	/* Create the Subscription-State header */
	if (expires >= 0) sub_state->expires_param = expires;
	if (state_str != NULL) pj_strdup(pool, &sub_state->sub_state, state_str);
	if (reason != NULL) pj_strdup(pool, &sub_state->reason_param, reason);
	if (retry_after >= 0) sub_state->retry_after = retry_after;
}
...
/*
 * Create and send a manual NOTIFY request.
 */
PJ_DEF(pj_status_t) pjsip_evsub_notify_force_subs_state(pjsip_evsub* sub,
												const int expires,			//Value -1 if it is not required
												pjsip_evsub_state state,
												...
												pjsip_tx_data** p_tdata)
{
	...
	modify_sub_state_hdr(tdata->pool, sub_state, expires, state_str, reason, retry_after);
	pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*)sub_state);
	...
	if (expires >= 0)
	{
		update_expires(sub, expires);
		/* Set UAS timeout timer, when state is not terminated. */
		if (sub->dst_state != PJSIP_EVSUB_STATE_TERMINATED) {
			set_timer(sub, TIMER_TYPE_UAS_TIMEOUT, expires);
		}
	}
						...
						pjsip_tx_data **p_tdata )
{
	...
	if (sub->state == PJSIP_EVSUB_STATE_TERMINATED && sub->expires->ivalue == 0)
	{
		pj_str_t reason = { "timeout", 7 };
		st = pjsip_evsub_notify(sub, sub->state, &sub->state_str, &reason, p_tdata);
	}
	else
	{
		st = pjsip_evsub_notify(sub, sub->state, &sub->state_str, NULL, p_tdata);
	}
				        ...
				        pjsip_event *event)
{
    ...
    struct dlgsub *dlgsub_head, *dlgsub;
    pjsip_evsub *sub;
	int dlgsub_count_event = 0;
	int dlgsub_count_no_event = 0;
	pj_bool_t dlgsub_found_event = PJ_FALSE;
	pj_bool_t dlgsub_found_no_event = PJ_FALSE;
	...
	dlgsub_count_event = 0;
	dlgsub_count_no_event = 0;
	dlgsub = dlgsub_head->next;
    ...
    while (dlgsub != dlgsub_head) {
		dlgsub_found_event = PJ_FALSE;
		dlgsub_found_no_event = PJ_FALSE;
		if (pj_stricmp(&dlgsub->sub->event->event_type, 
			       &event_hdr->event_type)==0)
		{
			/* Event type matched. 
			...
			*/
			...
			if (dlgsub_found_event == PJ_TRUE)
			{
				if (dlgsub_count_event > 0) dlgsub_count_event--;
				if (dlgsub_count_event == 0)
				{
					break;
				}
				else if (pjsip_evsub_get_state(dlgsub->sub) != PJSIP_EVSUB_STATE_TERMINATED)
				{
					break;
				}
			}
			if (dlgsub_found_no_event == PJ_TRUE)
			{
				if (dlgsub_count_no_event > 0) dlgsub_count_no_event--;
				if (dlgsub_count_no_event == 0)
				{
					break;
				}
				else if (pjsip_evsub_get_state(dlgsub->sub) != PJSIP_EVSUB_STATE_TERMINATED)
			      ...
			      pjsip_event *event )
{
	...
	if (tsx->status_code/100 == 2) {
		...
		if ((sub->state == PJSIP_EVSUB_STATE_ACTIVE || sub->state == PJSIP_EVSUB_STATE_PENDING) &&
			(tsx->status_code == PJSIP_SC_INTERVAL_TOO_BRIEF))
		{
			//Si la subscripcion esta activa y se recibe un 423 entonces se ignora
			return;
		}
			      ...
			      pjsip_event *event)
{
	...
	if (event_hdr && expires) {
		...
		if (sub->maximum_expires_as_uas > 0)
		{
			//Si esta configurado maximum_expires_as_uas, este es el valor que tiene prioridad como maximo
			if (sub->maximum_expires_as_uas < expires->ivalue)
			{
				//Si lo sobrepasa entonces este valor maximo es el que se establece
				sub->expires->ivalue = sub->maximum_expires_as_uas;
			}
			else
			{
				//Si no lo sobrepasa entonces toma el valor recibido en la cabecera expires
				sub->expires->ivalue = expires->ivalue;
			}
		}
		else 
		...
		if (sub->minimum_expires_as_uas > 0 && expires->ivalue > 0)
		{
			if (expires->ivalue < sub->minimum_expires_as_uas)
			{
				st_code = PJSIP_SC_INTERVAL_TOO_BRIEF;
			}
		}
	}
	 ...
	 * will send NOTIFY.
	 */
	if (st_code == 200) update_expires(sub, sub->expires->ivalue);
	...
	if (sub->user.on_rx_refresh && sub->call_cb && st_code == 200) {
	...
	if (status == PJ_SUCCESS) {
		...
		if (st_code == PJSIP_SC_INTERVAL_TOO_BRIEF)
		{
			pjsip_min_expires_hdr* min_expires_hdr;
			min_expires_hdr = pjsip_min_expires_hdr_create(tdata->pool, sub->minimum_expires_as_uas);
			if (min_expires_hdr) pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*)	pjsip_hdr_shallow_clone(tdata->pool, min_expires_hdr));
		}
		else
		{
			/* Add expires header: */
 ...
 */
PJ_DEF(void) set_timer_uas_timeout( pjsip_evsub *sub)
{
	set_timer(sub, TIMER_TYPE_UAS_TIMEOUT, sub->expires->ivalue);
}

/*
 * He visto que cuando no existe respuesta al primer mensaje SUBSCRIPTION y sin embargo el siguiente si obtiene
 * respuesta 200ok. El timer de refresco de la subscripcion no arranca. Creo esta funcion para arrancar el timer
 */
PJ_DEF(void) set_timer_uac_refresh_timeout( pjsip_evsub *sub)
{
	/* Start UAC refresh timer, only when we're not unsubscribing */
	if (sub->expires->ivalue != 0) {
		unsigned timeout = (sub->expires->ivalue > TIME_UAC_REFRESH) ?
...
PJ_DEF(void *) pjsip_evsub_get_event_hdr(pjsip_evsub *sub)
{
	return (void *) sub->event;
}

PJ_DEF(pj_pool_t *) pjsip_evsub_get_pool(pjsip_evsub *sub)
{
	return (pj_pool_t *) sub->pool;
}
