...
static pjsip_hdr *parse_hdr_event(pjsip_parse_ctx *ctx)
{
    ...
    pj_scan_get(ctx->scanner, &pc->pjsip_MULTI_TOKEN_SPEC, &hdr->event_type);
	while ((hdr->event_type.slen > 0) && (hdr->event_type.ptr[hdr->event_type.slen - 1] == ' ')) hdr->event_type.slen--;
