					   ...
					   pjsip_module *mod )
{
    ...
    PJ_LOG(6,(dlg->obj_name, "Session count inc to %d by %.*s",
...
PJ_DEF(pj_status_t) pjsip_dlg_try_inc_lock(pjsip_dialog *dlg)
{
    ...
    if (status != PJ_SUCCESS) {
	PJ_LOG(6,(dlg->obj_name, "pjsip_dlg_try_inc_lock() failed. status 0x%X", status));
	return status;
    }
...
/*
 * Destruye el dialogo sin bloqueos. EDU 23/02/2017
 * It may delete the dialog!
 */
PJ_DEF(void) pjsip_dlg_destroy(pjsip_dialog *dlg)
{
    PJ_ASSERT_ON_FAIL(dlg!=NULL, return);

    PJ_LOG(6,(dlg->obj_name, "Entering pjsip_dlg_destroy(), sess_count=%d", 
	      dlg->sess_count));
    ...
    PJ_LOG(6,(THIS_FILE, "Leaving pjsip_dlg_destroy() (dlg=%p)", dlg));
}
					   ...
					   pjsip_module *mod)
{
    ...
    PJ_LOG(6,(dlg->obj_name, "Session count dec to %d by %.*s",
					    ...
					    void *mod_data)
{
    ...
    if (dlg->WG67_version != NULL)
    {
        pjsip_hdr* hdr = pjsip_msg_find_hdr_by_name(msg, &dlg->WG67_version->name, NULL);
        if (hdr)
        {
            pj_list_erase(hdr);
        }
        pjsip_generic_string_hdr* wg67v = (pjsip_generic_string_hdr*)pjsip_hdr_clone(tdata->pool, dlg->WG67_version);
    ...
    {
        char wg67st[32];
        pj_ansi_snprintf(wg67st, sizeof(wg67st), "%.*s", dlg->WG67_version->hvalue.slen, dlg->WG67_version->hvalue.ptr);
        pjsip_tsx_set_WG67_version(tsx, wg67st);
    }
					     ...
					     pjsip_tx_data *tdata)
{
    ...
    if (dlg->WG67_version != NULL)
    {
        pjsip_hdr* hdr = pjsip_msg_find_hdr_by_name(tdata->msg, &dlg->WG67_version->name, NULL);
        if (hdr)
        {
            pj_list_erase(hdr);
        }
        pjsip_generic_string_hdr* wg67v = (pjsip_generic_string_hdr*)pjsip_hdr_clone(tdata->pool, dlg->WG67_version);
        pjsip_msg_add_hdr(tdata->msg, (pjsip_hdr*)wg67v);
    }
...
void pjsip_dlg_on_rx_request( pjsip_dialog *dlg, pjsip_rx_data *rdata )
{
	...
	/** 20160602. AGL. VOTER pide un 400 en vez de un 500 */
	pjsip_endpt_respond_stateless(dlg->endpt,
				      rdata, 400, &warn_text, NULL, NULL);
	return;
    }
	...
	if (contact)
	{
				      ...
				      contact->uri))
		{
			...
			dlg->remote.contact->focus = contact->focus;
    }
	 }
     ...
     */
    if (!processed && tsx && tsx->status_code < 200 && tsx->status_code != 100) 
	{
	pjsip_tx_data *tdata;
	
		/** 20160602. AGL. Para contestar al paquete OPTIONS */	
			     ...
			     pjsip_event *e )
{
	...
	/*  edu 20170220 Comento el if porque se aprecia pérdida de memoria*/
	/** 20160615 AGL */
	...
	if (tsx->state != PJSIP_TSX_STATE_TERMINATED)
		...
		pjsip_dlg_destroy(dlg);
}

PJ_DEF(pj_status_t) pjsip_dlg_set_WG67_version(pjsip_dialog* dlg, const char *WG67_version, pj_bool_t WG67_version_is_already_negotiated)
{
    pj_str_t hnames = pj_str("WG67-Version");
    pj_str_t hvalue = pj_str((char *) WG67_version);
    dlg->WG67_version = pjsip_generic_string_hdr_create(dlg->pool, &hnames, &hvalue);
    dlg->WG67_version_is_already_negotiated = WG67_version_is_already_negotiated;
    return PJ_SUCCESS;
}

PJ_DEF(pj_bool_t) pjsip_dlg_get_WG67_version(pjsip_dialog* dlg, pjsip_generic_string_hdr* WG67_version)
{
    if (WG67_version) WG67_version = dlg->WG67_version;
    return dlg->WG67_version_is_already_negotiated;
}
