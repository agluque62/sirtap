					       ...
					       pjsip_tx_data **p_tdata)
{
	...
	if (req_tdata->msg->type != PJSIP_REQUEST_MSG || req_tdata->msg->line.req.method.id != PJSIP_INVITE_METHOD)
		return PJ_EINVAL;
...
PJ_DEF(pj_status_t) pjsip_endpt_create_bye(pjsip_endpoint* endpt,
	const pjsip_tx_data* req_tdata,
	pjsip_tx_data** p_tdata)
{
	pjsip_tx_data* cancel_tdata = NULL;
					 ...
					 pj_ssize_t sent )
{
	...
	//Se establece el marcado DSCP de los paquetes SIP
	if (stateless_data->cur_transport)
	{
		pj_sock_t sip_socket;
		sip_socket = pjsip_udp_transport_get_socket(stateless_data->cur_transport);
		if (sip_socket != PJ_INVALID_SOCKET)
		{
			// Add socket to flow.
			BOOL QoSResult; 
			if (stateless_data->cur_transport->QoSHandle != INVALID_HANDLE_VALUE && stateless_data->cur_transport->QoSFlowId == 0)
			{								
				pj_sockaddr* a;
				a = (pj_sockaddr*)cur_addr;

				QoSResult = QOSAddSocketToFlow(stateless_data->cur_transport->QoSHandle, sip_socket, (PSOCKADDR)&a->ipv4,
					QOSTrafficTypeVoice, QOS_NON_ADAPTIVE_FLOW, (PQOS_FLOWID) &stateless_data->cur_transport->QoSFlowId);
				if (QoSResult != TRUE)
				{
					int werror = WSAGetLastError();
					PJ_LOG(3, (__FILE__, "ERROR: stateless_send_transport_cb: Changing DSCP mark QOSAddSocketToFlow failed to %s:%u. Error %d", 
						pj_inet_ntoa(a->ipv4.sin_addr), pj_ntohs(a->ipv4.sin_port), werror));
				}
				else
				{
					DWORD DSCPValue = PJSIP_SIP_PROTOCOL_DSCP;
					QoSResult = QOSSetFlow(stateless_data->cur_transport->QoSHandle, stateless_data->cur_transport->QoSFlowId, QOSSetOutgoingDSCPValue, sizeof(DSCPValue), &DSCPValue, 0, NULL);
					if (QoSResult != TRUE)
					{
						int werror = WSAGetLastError();
						if (werror == ERROR_ACCESS_DENIED)
						{
							PJ_LOG(3, (__FILE__, "ERROR:  stateless_send_transport_cb: QOSSetFlow failed to %s:%u. Error %d. Changing DSCP requires the calling application be a member of the Administrators or the Network Configuration Operators group", 
								pj_inet_ntoa(a->ipv4.sin_addr), pj_ntohs(a->ipv4.sin_port), werror));
						}
						else
						{
							PJ_LOG(3, (__FILE__, "ERROR:  stateless_send_transport_cb: QOSSetFlow failed Changing DSCP mark to %s:%u. Error %d", 
								pj_inet_ntoa(a->ipv4.sin_addr), pj_ntohs(a->ipv4.sin_port), werror));
						}
					}
				}
				       ...
				       const pjsip_server_addresses *addr )
{
	...
	//Se establece el marcado DSCP de los paquetes SIP
	if (send_state->cur_transport)
	{
		pj_sock_t sip_socket;
		sip_socket = pjsip_udp_transport_get_socket(send_state->cur_transport);
		if (sip_socket != PJ_INVALID_SOCKET)
		{
			// Add socket to flow.
			BOOL QoSResult;
			if (send_state->cur_transport->QoSHandle != INVALID_HANDLE_VALUE && send_state->cur_transport->QoSFlowId == 0)
			{
				pj_sockaddr* a;
				a = (pj_sockaddr*) &addr->entry[0].addr;

				QoSResult = QOSAddSocketToFlow(send_state->cur_transport->QoSHandle, sip_socket, (PSOCKADDR)&a->ipv4,
					QOSTrafficTypeVoice, QOS_NON_ADAPTIVE_FLOW, (PQOS_FLOWID)&send_state->cur_transport->QoSFlowId);
				if (QoSResult != TRUE)
				{
					int werror = WSAGetLastError();
					PJ_LOG(3, (__FILE__, "ERROR: send_response_resolver_cb: Changing DSCP mark QOSAddSocketToFlow failed to %s:%u. Error %d",
						pj_inet_ntoa(a->ipv4.sin_addr), pj_ntohs(a->ipv4.sin_port), werror));
				}
				else
				{
					DWORD DSCPValue = PJSIP_SIP_PROTOCOL_DSCP;
					QoSResult = QOSSetFlow(send_state->cur_transport->QoSHandle, send_state->cur_transport->QoSFlowId, QOSSetOutgoingDSCPValue, sizeof(DSCPValue), &DSCPValue, 0, NULL);
					if (QoSResult != TRUE)
					{
						int werror = WSAGetLastError();
						if (werror == ERROR_ACCESS_DENIED)
						{
							PJ_LOG(3, (__FILE__, "ERROR:  send_response_resolver_cb: QOSSetFlow failed to %s:%u. Error %d. Changing DSCP requires the calling application be a member of the Administrators or the Network Configuration Operators group",
								pj_inet_ntoa(a->ipv4.sin_addr), pj_ntohs(a->ipv4.sin_port), werror));
						}
						else
						{
							PJ_LOG(3, (__FILE__, "ERROR:  send_response_resolver_cb: QOSSetFlow failed Changing DSCP mark to %s:%u. Error %d",
								pj_inet_ntoa(a->ipv4.sin_addr), pj_ntohs(a->ipv4.sin_port), werror));
						}
					}
				}
	...
	if (send_state->cur_transport)
	{
		...
		if (sip_socket != PJ_INVALID_SOCKET)
		{
			...
			if (send_state->cur_transport->QoSHandle != INVALID_HANDLE_VALUE && send_state->cur_transport->QoSFlowId == 0)
			{
				...
				{
					int werror = WSAGetLastError();
					PJ_LOG(3, (__FILE__, "ERROR: pjsip_endpt_send_response: Changing DSCP mark QOSAddSocketToFlow failed to %s:%u. Error %d",
						pj_inet_ntoa(a->ipv4.sin_addr), pj_ntohs(a->ipv4.sin_port), werror));
				}
				else
				{
					...
					if (QoSResult != TRUE)
					{
						...
						if (werror == ERROR_ACCESS_DENIED)
						{
							PJ_LOG(3, (__FILE__, "ERROR:  pjsip_endpt_send_response: QOSSetFlow failed to %s:%u. Error %d. Changing DSCP requires the calling application be a member of the Administrators or the Network Configuration Operators group",
								pj_inet_ntoa(a->ipv4.sin_addr), pj_ntohs(a->ipv4.sin_port), werror));
						}
						else
						{
							PJ_LOG(3, (__FILE__, "ERROR:  pjsip_endpt_send_response: QOSSetFlow failed Changing DSCP mark to %s:%u. Error %d",
								pj_inet_ntoa(a->ipv4.sin_addr), pj_ntohs(a->ipv4.sin_port), werror));
						}
					}
				}
