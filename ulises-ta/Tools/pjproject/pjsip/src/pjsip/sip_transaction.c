/* $Id: sip_transaction.c 2739 2009-06-03 10:19:44Z bennylp $ */
/* 
...
static pj_time_val t1_timer_val = { PJSIP_T1_TIMEOUT/1000, PJSIP_T1_TIMEOUT%1000 };
static pj_time_val t2_timer_val = { PJSIP_T2_TIMEOUT/1000, PJSIP_T2_TIMEOUT%1000 };
static pj_time_val t4_timer_val = { PJSIP_T4_TIMEOUT/1000, PJSIP_T4_TIMEOUT%1000 };
static pj_time_val td_timer_val = { PJSIP_TD_TIMEOUT/1000, PJSIP_TD_TIMEOUT%1000 };
static pj_time_val timeout_timer_val = { PJSIP_TSX_TIMEOUT/1000,PJSIP_TSX_TIMEOUT%1000 };
...
PJ_DEF_DATA(const pjsip_method) mypjsip_refer_method = 
{
...
/* Prototypes. */
static pj_status_t lock_tsx(pjsip_transaction *tsx, struct tsx_lock_data *lck);
...
PJ_DEF(pj_status_t) pjsip_tsx_layer_init_module(pjsip_endpoint *endpt)
{
	...
	timeout_timer_val.sec  = pjsip_cfg()->tsx.tsx_tout / 1000;
	timeout_timer_val.msec = pjsip_cfg()->tsx.tsx_tout % 1000;
...
static void tsx_timer_callback( pj_timer_heap_t *theap, pj_timer_entry *entry)
{
	...
	if (lock_tsx(tsx, &lck) != PJ_SUCCESS) 
	{
		PJ_LOG(3,(THIS_FILE, "tsx_timer_callback: lock_tsx returns error"));
		return;
	}
													  ...
													  pjsip_transaction **p_tsx)
{
	...
	/* Lock transaction. */
	status = lock_tsx(tsx, &lck);
	if (status != PJ_SUCCESS) 
	{
		PJ_LOG(3,(THIS_FILE, "pjsip_tsx_create_uac: lock_tsx returns error"));
		tsx_destroy(tsx);
		return status;
	}
	...
	{
		static pj_str_t subjectHdr = { "Subject", 7 };
		static pj_str_t subjects[] = { { "IA call", 7 }, { "monitoring", 10 }, { "G/G monitoring", 10 }, { "A/G monitoring", 10 }, { "DA/IDA call", 11}, { "radio", 5 } };
		pjsip_generic_string_hdr * subject = (pjsip_generic_string_hdr*)pjsip_msg_find_hdr_by_name(msg, &subjectHdr, NULL);

		tsx->inv_proceeding_tout = 60000;

		if (subject != NULL)
		{
			if (pj_stricmp(&subject->hvalue, &subjects[0]) == 0)
			{
				tsx->inv_proceeding_tout = pjsip_cfg()->tsx.inv_proceeding_ia_tout;
			}
			else if ((pj_stricmp(&subject->hvalue, &subjects[1]) == 0) || 
				(pj_stricmp(&subject->hvalue, &subjects[2]) == 0) ||
				(pj_stricmp(&subject->hvalue, &subjects[3]) == 0))
			{
				tsx->inv_proceeding_tout = pjsip_cfg()->tsx.inv_proceeding_monitoring_tout;
			}
			else if (pj_stricmp(&subject->hvalue, &subjects[4]) == 0)
			{
				tsx->inv_proceeding_tout = pjsip_cfg()->tsx.inv_proceeding_dia_tout;
			}
			else if (pj_stricmp(&subject->hvalue, &subjects[5]) == 0)
			{
				tsx->inv_proceeding_tout = pjsip_cfg()->tsx.inv_proceeding_rd_tout;
			}
		}
	}
													  ...
													  pjsip_transaction **p_tsx)
{
	...
	*/
	...
	if (status != PJ_SUCCESS) 
	{
		PJ_LOG(3,(THIS_FILE, "pjsip_tsx_create_uas: lock_tsx returns error"));
		tsx_destroy(tsx);
		return status;
	}
														  ...
														  const pjsip_tpselector *sel)
{
	...
	status = status = lock_tsx(tsx, &lck);
	if (status != PJ_SUCCESS) 
	{
		PJ_LOG(3,(THIS_FILE, "pjsip_tsx_set_transport: lock_tsx returns error"));
		return status;
	}
...
PJ_DEF(pj_status_t) pjsip_tsx_terminate( pjsip_transaction *tsx, int code )
{
	...
	if (status != PJ_SUCCESS) 
	{
		PJ_LOG(3,(THIS_FILE, "pjsip_tsx_terminate: lock_tsx returns error"));
		return status;
	}
...
PJ_DEF(pj_status_t) pjsip_tsx_stop_retransmit(pjsip_transaction *tsx)
{
	...
	if (status != PJ_SUCCESS) 
	{
		PJ_LOG(3,(THIS_FILE, "pjsip_tsx_stop_retransmit: lock_tsx returns error"));
		return status;
	}
	/* Cancel retransmission timer. */
	if (tsx->retransmit_timer.id != 0) {
														...
														unsigned millisec)
{
	...
	if (status != PJ_SUCCESS) 
	{
		PJ_LOG(3,(THIS_FILE, "ERROR: pjsip_tsx_set_timeout: lock_tsx returns error"));
		return status;
	}
													...
													pjsip_tx_data *tdata )
{
	...
	if (tsx->WG67_version != NULL)
	{
		pjsip_msg* msg;
		msg = tdata->msg;
		pjsip_hdr* hdr = pjsip_msg_find_hdr_by_name(msg, &tsx->WG67_version->name, NULL);
		if (hdr)
		{
			pj_list_erase(hdr);
		}
		pjsip_generic_string_hdr* wg67v = (pjsip_generic_string_hdr*)pjsip_hdr_clone(tdata->pool, tsx->WG67_version);
		pjsip_msg_add_hdr(msg, (pjsip_hdr*)wg67v);
	}
	...
	if (status != PJ_SUCCESS) 
	{
		PJ_LOG(3,(THIS_FILE, "pjsip_tsx_send_msg: lock_tsx returns error"));
		return status;
	}
										  ...
										  pjsip_rx_data *rdata)
{
	...
	if (status != PJ_SUCCESS) 
	{
		PJ_LOG(3,(THIS_FILE, "pjsip_tsx_recv_msg: lock_tsx returns error"));
		return;
	}
	status = (*tsx->state_handler)(tsx, &event);
	unlock_tsx(tsx, &lck);
										...
										pj_ssize_t sent, pj_bool_t *cont )
{
	...
	*cont = PJ_FALSE;
	...
	if (status != PJ_SUCCESS) 
	{
		PJ_LOG(3,(THIS_FILE, "send_msg_callback: lock_tsx returns error"));
		return;
	}
										 ...
										 pj_ssize_t sent)
{
	if (sent < 0) {
		...
		if (status != PJ_SUCCESS) 
		{
			PJ_LOG(3,(THIS_FILE, "transport_callback: lock_tsx returns error"));
			return;
		}
				   ...
				   const pjsip_transport_state_info *info)
{
	...
	if (status != PJ_SUCCESS) 
	{
		PJ_LOG(3,(THIS_FILE, "tsx_tp_state_callback: lock_tsx returns error"));
		return;
	}
																  ...
																  pjsip_tx_data *tdata)
{
	...
	if (status != PJ_SUCCESS) 
	{
		PJ_LOG(3,(THIS_FILE, "pjsip_tsx_retransmit_no_state: lock_tsx returns error"));
		return status;
	}
	if (tdata == NULL) {
		tdata = tsx->last_tx;
												 ...
												 pjsip_event *event )
{
	...
	if (tsx->role == PJSIP_ROLE_UAS) {
		...
		*/
		...
		if (tdata->msg->type == PJSIP_REQUEST_MSG && 
			tsx->method.id == PJSIP_OTHER_METHOD && 
			(pj_strcmp2(&tsx->method.name, "PUBLISH") == 0 || pj_strcmp2(&tsx->method.name, "SUBSCRIBE") == 0))
		{
			//Se ha visto que el servidor de presencia es lento para enviar el 200OK. 
			//Por tanto, para PUBLISH multiplicamos el time-out
			//Hacemos lo mismo para SUBSCRIBE
			unsigned int tsx_tout = pjsip_cfg()->tsx.tsx_tout * 8;
			pj_time_val time_val;
			time_val.sec = tsx_tout / 1000;
			time_val.msec = tsx_tout % 1000;
			tsx->timeout_timer.id = TIMER_ACTIVE;
			pjsip_endpt_schedule_timer(tsx->endpt, &tsx->timeout_timer,
				&time_val);
		}
													 ...
													 pjsip_event *event )
{
		...
		event->body.timer.entry == &tsx->timeout_timer) 
	{
		...
		if (tsx->method.id == PJSIP_INVITE_METHOD && tsx->last_tx->msg->line.req.method.id == PJSIP_INVITE_METHOD && !pjsip_endpt_Is_ETM(tsx->endpt))
		{			
			pj_str_t name = pj_str("Subject");
			pjsip_generic_string_hdr *subject = (pjsip_generic_string_hdr *)pjsip_msg_find_hdr_by_name(tsx->last_tx->msg, &name, NULL);
			if (subject != NULL)
			{
				if (pj_stricmp2(&subject->hvalue, "radio") == 0)
				{
					/*
						Resuelve incidencia de Vigo.Un transmisor principal M + N se queda disabled porque temporalmente se detecta que tiene 2 sesiones.
						Si no hay respuesta al INVITE del tipo radio enviamos BYE porque se han visto problemas
						en las radios Jotron, el cual manda el 200OK despues del tsx timeout e incrementan el numero de sesiones
						temporalmente.
						Esta versión manda BYE para forzar a la radio a que mate la sesion antes de reintentar otra
					*/

					pj_status_t status;
					pjsip_tx_data* bye_tdata = NULL;

					status = pjsip_endpt_create_bye(tsx->endpt, tsx->last_tx, &bye_tdata);
					if (status == PJ_SUCCESS)
					{
						status = tsx_send_msg(tsx, bye_tdata);
						if (status == PJ_SUCCESS)
						{
							if (bye_tdata != tsx->last_tx) 
							{
								pjsip_tx_data_dec_ref(tsx->last_tx);
								tsx->last_tx = bye_tdata;
							}
						}
					}
				}
		...
		*/
		if (code >= 200) {
			...
			*/
			if (tsx->method.id == PJSIP_INVITE_METHOD) {
				pj_time_val tout_val;
				tout_val.sec = tsx->inv_proceeding_tout / 1000;
				tout_val.msec = tsx->inv_proceeding_tout % 1000;
				...
				tsx->timeout_timer.id = TIMER_ACTIVE;
				pjsip_endpt_schedule_timer( tsx->endpt, &tsx->timeout_timer, &tout_val);
															  ...
															  pjsip_event *event)
{
	...
	if (PJSIP_IS_STATUS_IN_CLASS(tsx->status_code, 100)) {
		...
		*/
		...
		else if (pjsip_method_cmp(&tsx->method, &mypjsip_refer_method)==0)
		{
			//Si se ha recibido el 202 como respuesta a un REFER, entonces se termina
			//la transaccion
			tsx_set_state( tsx, PJSIP_TSX_STATE_TERMINATED, 
				PJSIP_EVENT_RX_MSG, event->body.rx_msg.rdata );
		}
		...
		event->body.timer.entry == &tsx->timeout_timer) {
			...
			if ((tsx->method.id == PJSIP_INVITE_METHOD) && (tsx->status_code >= 100) && 
				(tsx->last_tx->msg->line.req.method.id != PJSIP_CANCEL_METHOD)) {

				/*Si se produce el timeout y el metodo de la transaccion es un invite, pero el ultimo paquete
				sip no es un CANCEL, entonces se envia el cancel esperando el OK y el 487. 
				Para ello arranca otra vez el tsx->timeout_timer.
				Si transcurrido este ultimo timeout porque no se ha recibido respuesta al CANCEL,
				y, por tanto, el paquete sip ultimo a sido el cancel entonces ya
				no se espera mas y se pasa a estado TERMINATED*/

				pj_status_t status;
				pjsip_tx_data *cancel_tdata = NULL;
				pj_time_val timeout;

				status = pjsip_endpt_create_cancel(tsx->endpt, tsx->last_tx,
					&cancel_tdata);
				if (status != PJ_SUCCESS)
					return status;

				status = tsx_send_msg(tsx, cancel_tdata);
				if (status != PJ_SUCCESS)
					return status;

				if (cancel_tdata != tsx->last_tx) {
					pjsip_tx_data_dec_ref(tsx->last_tx);
					tsx->last_tx = cancel_tdata;
					/* This is a bug.
															 ...
															 pjsip_event *event)
{
	...
	if (event->type == PJSIP_EVENT_RX_MSG) {
		...
		/** 2016062. AGL. VOTER. Chequear CSeq. si es mala no detener la retransmision */
		{
			pj_str_t name = pj_str("CSeq");
			pjsip_cseq_hdr *cseq_hdr = (pjsip_cseq_hdr *) pjsip_msg_find_hdr_by_name(msg, &name, NULL);
			if (cseq_hdr != NULL)
			{
				if (cseq_hdr->method.id != PJSIP_ACK_METHOD)
				{
					return PJSIP_EINVALIDMETHOD;
				}
			}
...
PJ_DEF(pj_status_t) pjsip_tsx_set_WG67_version(pjsip_transaction* tsx, const char* WG67_version)
{
	pj_str_t hnames = pj_str("WG67-Version");
	pj_str_t hvalue = pj_str((char*)WG67_version);
	tsx->WG67_version = pjsip_generic_string_hdr_create(tsx->pool, &hnames, &hvalue);
	return PJ_SUCCESS;
}

PJ_DEF(pjsip_generic_string_hdr*) pjsip_tsx_get_WG67_version(pjsip_transaction* tsx)
{
	return tsx->WG67_version;
}
