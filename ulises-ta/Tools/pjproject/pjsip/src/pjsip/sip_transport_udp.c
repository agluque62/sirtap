/* $Id: sip_transport_udp.c 3110 2010-02-25 11:58:19Z nanang $ */
/* 
				  ...
				  pj_ssize_t bytes_read)
{
	 ...
	 */
	{
	    ...
	    rdata_index = (unsigned)(unsigned long)(pj_ssize_t)rdata->tp_info.tp_data;
...
static pj_status_t udp_destroy( pjsip_transport *transport )
{
    ...
    */
	...
	if (tp->base.QoSHandle != INVALID_HANDLE_VALUE)
	{
		PJ_LOG(3, (__FILE__, "DEBUG: udp_destroy: Destruye QoSHandle %p pjsip_transport 0x%X",
			tp->base.QoSHandle, tp->base));

		QOSCloseHandle(tp->base.QoSHandle);
		tp->base.QoSHandle = INVALID_HANDLE_VALUE;
	}
				     ...
				     pjsip_transport **p_transport)
{
	...
	QoSResult = QOSCreateHandle(&Version, &tp->base.QoSHandle);
	if (QoSResult != TRUE)
	{
		tp->base.QoSHandle = INVALID_HANDLE_VALUE;
		PJ_LOG(3, (__FILE__, "ERROR: transport_attach: QOSCreateHandle failed pjsip_transport 0x%X", tp->base));
		goto on_error;
	}
	else
	{
		PJ_LOG(3, (__FILE__, "DEBUG: transport_attach: Crea QoSHandle 0x%p pjsip_transport 0x%X",
			tp->base.QoSHandle, tp->base));
	}
