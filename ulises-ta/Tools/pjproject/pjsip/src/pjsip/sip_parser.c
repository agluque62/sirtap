...
static pj_status_t init_parser()
{
	...
	*/
	...
	status = pj_cis_dup(&pconst.pjsip_MULTI_TOKEN_SPEC, &pconst.pjsip_TOKEN_SPEC);
	PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
	pj_cis_add_str( &pconst.pjsip_MULTI_TOKEN_SPEC, " ");
...
pjsip_uri *int_parse_uri_or_name_addr( pj_scanner *scanner, pj_pool_t *pool,
												  unsigned opt)
{
	pjsip_uri *uri;
	int is_name_addr = 0;
