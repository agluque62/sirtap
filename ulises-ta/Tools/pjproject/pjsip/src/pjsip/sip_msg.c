...
PJ_DEF_DATA(const pjsip_method) pjsip_invite_method = { PJSIP_INVITE_METHOD, { "INVITE",6 }};
PJ_DEF_DATA(const pjsip_method) pjsip_cancel_method = { PJSIP_CANCEL_METHOD, { "CANCEL",6 }};
PJ_DEF_DATA(const pjsip_method) pjsip_ack_method = { PJSIP_ACK_METHOD, { "ACK",3}};
PJ_DEF_DATA(const pjsip_method) pjsip_bye_method = { PJSIP_BYE_METHOD, { "BYE",3}};
PJ_DEF_DATA(const pjsip_method) pjsip_register_method = { PJSIP_REGISTER_METHOD, { "REGISTER", 8}};
PJ_DEF_DATA(const pjsip_method) pjsip_options_method = { PJSIP_OPTIONS_METHOD, { "OPTIONS",7}};
...
static int init_status_phrase()
{
	...
	pj_strset2( &status_phrase[183], "Intrusion in progress");
				   ...
				   pjsip_hdr_e hdr_type, const void *start)
{
    ...
    for (; hdr!=end; hdr = hdr->next) 
	{
		if (hdr->type == hdr_type)
			return (void*)hdr;
		/** 20160530 AGL A veces se queda en bucle infinito... */
		if (++cnt == 100)
			break;
    }
    return NULL;
...
PJ_DEF(const pj_str_t*) pjsip_get_status_text(int code)
{
    ...
    return (code>=100 && code<(int)(sizeof(status_phrase)/sizeof(status_phrase[0]))) ? &status_phrase[code] : &status_phrase[0];
}
...
/** */
static pjsip_generic_array_hdr* pjsip_generic_array_hdr_clone( pj_pool_t *pool, const pjsip_generic_array_hdr *hdr);
/** */
static pjsip_generic_array_hdr* pjsip_generic_array_hdr_shallow_clone( pj_pool_t *pool, const pjsip_generic_array_hdr *hdr);
/** */
...
/** */
static pjsip_cseq_hdr* pjsip_cseq_hdr_clone( pj_pool_t *pool, const pjsip_cseq_hdr *rhs)
{
...
static pjsip_cseq_hdr* pjsip_cseq_hdr_shallow_clone( pj_pool_t *pool, const pjsip_cseq_hdr *rhs )
{
 ...
 * Expires header. AGL
 */
PJ_DEF(pjsip_expires_hdr*) pjsip_expires_hdr_init( pj_pool_t *pool, void *mem, int value)
{
...
/** */
PJ_DEF(pjsip_expires_hdr*) pjsip_expires_hdr_create( pj_pool_t *pool, int value )
{
 ...
 */
/** */
static int pjsip_fromto_hdr_print( pjsip_fromto_hdr *hdr, char *buf, pj_size_t size);
static pjsip_fromto_hdr* pjsip_fromto_hdr_clone( pj_pool_t *pool, const pjsip_fromto_hdr *hdr);
static pjsip_fromto_hdr* pjsip_fromto_hdr_shallow_clone( pj_pool_t *pool, const pjsip_fromto_hdr *hdr);
...
/** */
static pjsip_fromto_hdr* pjsip_fromto_hdr_clone( pj_pool_t *pool, const pjsip_fromto_hdr *rhs)
{
...
/** */
static pjsip_fromto_hdr* pjsip_fromto_hdr_shallow_clone( pj_pool_t *pool, const pjsip_fromto_hdr *rhs)
{
 ...
 * Retry-After header.
 */
static int pjsip_retry_after_hdr_print(pjsip_retry_after_hdr *r, char *buf, pj_size_t size );
static pjsip_retry_after_hdr* pjsip_retry_after_hdr_clone(pj_pool_t *pool, const pjsip_retry_after_hdr *r);
static pjsip_retry_after_hdr* pjsip_retry_after_hdr_shallow_clone(pj_pool_t *pool, const pjsip_retry_after_hdr *r );
 ...
 * Supported header.
 */
PJ_DEF(pjsip_supported_hdr*) pjsip_supported_hdr_init( pj_pool_t *pool, void *mem )
{
