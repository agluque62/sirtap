...
struct pjsip_endpoint
{
    ...
    int options_answer_code;    //Es el codigo de la respuesta para todos los siguientes mensajes OPTIONS. 
                                ...
                                //Si es #SIP_ENDPOINT_OPTIONS_ANSWERS_DEFAULT. Entonces funciona como por defecto

    pj_bool_t ETM;              //Si es true, entonces se trata del producto ETM
						...
						const pj_str_t tags[])
{
    ...
    /* Find the header. */
    hdr = (pjsip_generic_array_hdr*) pjsip_endpt_get_capability(endpt, htype, hname);
...
/*
 * Modify or add Max-forward header.
 */
PJ_DEF(void) pjsip_endpt_modify_max_forward_header(pjsip_endpoint* endpt, pj_int32_t value)
{
    pjsip_hdr* hdr, * end = &endpt->req_hdr;
    pj_bool_t found = PJ_FALSE;
    pjsip_max_fwd_hdr* mf_hdr;

    hdr = endpt->req_hdr.next;
    for (; hdr != end; hdr = hdr->next)
    {
        if (hdr->type == PJSIP_H_MAX_FORWARDS)
        {
            mf_hdr = (pjsip_max_fwd_hdr *) hdr;
            mf_hdr->ivalue = value;
            found = PJ_TRUE;
            break;
        }            
    ...
    if (found == PJ_FALSE)
    {
        mf_hdr = pjsip_max_fwd_hdr_create(endpt->pool, value);
...
/*
 * Modify options_answer_uri_without_user
 */
PJ_DEF(void) pjsip_endpt_modify_options_answer(pjsip_endpoint* endpt, int value)
{
    endpt->options_answer_code = value;
    return;
}

/*
*   Get options_answer_uri_without_user
*/
PJ_DEF(int) pjsip_endpt_get_options_answer(pjsip_endpoint * endpt)
{
    return endpt->options_answer_code;
}
                                       ...
                                       pjsip_endpoint **p_endpt)
{
    ...
    *p_endpt = NULL;
    ...
    endpt->options_answer_code = SIP_ENDPOINT_OPTIONS_ANSWERS_DEFAULT;
...
PJ_DEF(void) pjsip_endpt_Set_Ed137_version(pjsip_endpoint* endpt, char ED137Radioversion, char ED137Phoneversion)
{
    endpt->ED137PhoneVersion = ED137Phoneversion;
    endpt->ED137RadioVersion = ED137Radioversion;
}

PJ_DEF(void) pjsip_endpt_Get_Ed137_version(pjsip_endpoint* endpt, char* ED137Radioversion, char* ED137Phoneversion)
{
    if (ED137Radioversion) *ED137Radioversion = endpt->ED137RadioVersion;
    if (ED137Phoneversion) *ED137Phoneversion = endpt->ED137PhoneVersion;
}
...
#define Pjsip_endpt_Get_msg_ED137Version_MAXVERSIZE (64)
    char versions[Pjsip_endpt_Get_msg_ED137Version_MAXVER][Pjsip_endpt_Get_msg_ED137Version_MAXVERSIZE];
...
PJ_DEF(pj_bool_t) pjsip_endpt_Is_ETM(pjsip_endpoint* endpt)
{
    return endpt->ETM;
}
