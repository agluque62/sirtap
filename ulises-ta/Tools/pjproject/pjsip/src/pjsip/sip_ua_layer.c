					   ...
					   pjsip_dialog *dlg )
{
	...
	if (dlg_set) {
	    ...
	    pj_hash_set_np(mod_ua.dlg_table, 
			   dlg->local.info->tag.ptr, (unsigned)dlg->local.info->tag.slen,
					     ...
					     pjsip_dialog *dlg )
{
	...
	pj_hash_set(NULL, mod_ua.dlg_table, dlg->local.info->tag.ptr,
		    (unsigned)dlg->local.info->tag.slen, dlg->local.tag_hval, NULL);
					   ...
					   pj_bool_t lock_dialog)
{
    ...
    dlg_set = (struct dlg_set*)
    	      pj_hash_get(mod_ua.dlg_table, local_tag->ptr, (unsigned)local_tag->slen,
			  NULL);
    if (dlg_set == NULL) {
	/* Not found */
	pj_mutex_unlock(mod_ua.mutex);
...
static struct dlg_set *find_dlg_set_for_msg( pjsip_rx_data *rdata )
{
	...
	/* Lookup the dialog set. */
	dlg_set = (struct dlg_set*)
		  pj_hash_get(mod_ua.dlg_table, tag->ptr, (unsigned)tag->slen, NULL);
	return dlg_set;
    }
}
...
retry_on_deadlock:
	 ...
	 */
	pj_mutex_unlock(mod_ua.mutex);
	pj_thread_sleep(retry);
	retry++;	
	if (retry == 100) retry = 0;
	goto retry_on_deadlock;
    }
