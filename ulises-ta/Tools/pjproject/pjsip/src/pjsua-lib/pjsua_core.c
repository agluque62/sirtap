/* $Id: pjsua_core.c 2744 2009-06-04 15:29:14Z bennylp $ */
/* 
...
static pj_bool_t options_on_rx_request(pjsip_rx_data *rdata)
{
	...
	int st_code = 200;
	int options_answer = SIP_ENDPOINT_OPTIONS_ANSWERS_DEFAULT;
	...
	{
		if (pj_stricmp(&rdata->msg_info.msg->body->content_type.type, &STR_APPLICATION) == 0 &&
			pj_strnicmp(&rdata->msg_info.msg->body->content_type.subtype, &STR_CFWR_XML, STR_CFWR_XML.slen) == 0)
		{
			//Se trata de un OPTIONS para la negociacion de Call Forward.
			//Se procesa en otro modulo (OptionsMod)
			return PJ_FALSE;
		}
	}
	...
	options_answer = pjsip_endpt_get_options_answer(pjsua_var.endpt);
	if (options_answer == SIP_ENDPOINT_OPTIONS_ANSWERS_DEFAULT)
	{
    ...
    status = pjsip_endpt_create_response(pjsua_var.endpt, rdata, st_code, NULL, &tdata);
    ...
    if (pjsua_var.calls[0].med_tp && st_code == 200) {
    ...
    if (status == PJ_SUCCESS) {
		if (pj_app_cbs.on_create_sdp)
			pj_app_cbs.on_create_sdp(NULL, PJSUA_INVALID_ID, sdp, rdata);
...
PJ_DEF(pj_status_t) pjsua_init( const pjsua_config *ua_cfg, const pjsua_logging_config *log_cfg, const pjsua_media_config *media_cfg)
{
	...
	/* Add INFO in Allow header */
    pjsip_endpt_add_capability(pjsua_var.endpt, NULL, PJSIP_H_ALLOW, NULL, 1, &STR_INFO);
