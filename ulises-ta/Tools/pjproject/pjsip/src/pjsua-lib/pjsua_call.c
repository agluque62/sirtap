/* $Id: pjsua_call.c 2752 2009-06-05 10:14:20Z bennylp $ */
/* 
...
static void pjsua_call_on_reinvite_received(pjsip_inv_session* inv, const pjsip_event* e, pjsip_tx_data* tdata, int* returned_code, pj_str_t** cause);

static pj_status_t pjsua_call_on_send_ack(pjsip_inv_session* inv, pjsip_rx_data* rdata, pjsip_event* e);
...
static void reset_call(pjsua_call_id id)
{
	...
	call->monitoring = PJ_FALSE;
	call->incoming_rdata = NULL;
}
...
pj_status_t pjsua_call_subsys_init(const pjsua_config *cfg)
{
	...
	inv_cb.on_reinvite_received = &pjsua_call_on_reinvite_received;
													  ...
													  pjsua_call_id *p_call_id)
{
	...
	pjsip_tx_data *tdata;
	pj_status_t status;
	pj_str_t focus_param = { ";isfocus", 8 };
	char from_uri[256];
	pj_str_t pjstr_from_uri = pj_str("");
	pjsip_uri* pjsip_uri_fromuri = NULL;
	pjsip_uri* pjsip_reg_uri = NULL;
	pjsip_sip_uri *url_fromuri = NULL;
	pjsip_sip_uri *url_reguri = NULL;
	pj_bool_t invite_hacia_proxy = PJ_FALSE;
	pj_str_t gSubjectHdr = { "Subject", 7 };
	...
	options &= ~PJSIP_INV_VIA_PROXY;		//Lo borramos por si acaso
	...
	*/
	...
	if (!acc->valid) 
	{
		pjsua_perror(THIS_FILE, "Unable to make call because account is not valid", PJ_EINVALIDOP);
		PJSUA_UNLOCK();
	...
	*/
	...
	/** AGL */
	PJ_LOG(5,(THIS_FILE, "Making call with acc #%d to %.*s", acc_id, (int)dest_uri->slen, dest_uri->ptr));
	...
	*/
	...
	{
		contact = acc->contact;
		if (options & PJSUA_CALL_CONFERENCE)
		{
			contact.ptr = (char*)pj_pool_alloc(tmp_pool, PJSIP_MAX_URL_SIZE);
			contact.slen = pj_ansi_snprintf(contact.ptr, PJSIP_MAX_URL_SIZE,
				"%.*s%.*s",
				(int)acc->contact.slen,
				acc->contact.ptr,
				(int)focus_param.slen,
				focus_param.ptr);
		}
	} 
	else 
	{
		status = pjsua_acc_create_uac_contact(tmp_pool, &contact,
			acc_id, dest_uri, (options & PJSUA_CALL_CONFERENCE ? &focus_param : NULL));
		if (status != PJ_SUCCESS) 
		{
	...
	if (invite_hacia_proxy)
	{
		//Se sustituye el host de la uri del From por la del proxy a la que se registra
		pjstr_from_uri.slen = 0;
		pjsip_uri_fromuri = pjsip_parse_uri(tmp_pool, acc->cfg.id.ptr, acc->cfg.id.slen, 0);
		pjsip_reg_uri = pjsip_parse_uri(tmp_pool, acc->cfg.reg_uri.ptr, acc->cfg.reg_uri.slen, 0);
		if ((pjsip_uri_fromuri != NULL) && (pjsip_reg_uri != NULL))
		{
			url_fromuri = (pjsip_sip_uri*)pjsip_uri_get_uri(pjsip_uri_fromuri);
			url_reguri = (pjsip_sip_uri*)pjsip_uri_get_uri(pjsip_reg_uri);
			pj_strdup(tmp_pool, &url_fromuri->host, &url_reguri->host);
			url_fromuri->port = url_reguri->port;

			from_uri[0] = '\0';
			pjsip_uri_print(PJSIP_URI_IN_FROMTO_HDR, pjsip_uri_fromuri, from_uri, sizeof(from_uri));
			pjstr_from_uri = pj_str((char *) &from_uri);
		}
	}

	/* Create outgoing dialog: */
	if (invite_hacia_proxy && pjstr_from_uri.slen > 0)
	{
		//En este caso el From lleva la ip del proxy en la URI
		status = pjsip_dlg_create_uac( pjsip_ua_instance(), 
			&pjstr_from_uri, &contact,
	...
	*/
	...
	// PJ_LOG(3,(THIS_FILE, "Making call. Session Timer %d/%d ...", acc->cfg.timer_setting.min_se, acc->cfg.timer_setting.sess_expires));
	...
	*/
	...
	wg67ver_hdr = (pjsip_generic_string_hdr*)pjsip_msg_find_hdr_by_name(tdata->msg, &wg67ver, NULL);
	if (wg67ver_hdr != NULL)
	{
		char wg67st[32];
		pj_ansi_snprintf(wg67st, sizeof(wg67st), "%.*s", wg67ver_hdr->hvalue.slen, wg67ver_hdr->hvalue.ptr);
		pjsip_dlg_set_WG67_version(dlg, wg67st, PJ_FALSE);
	}
	
	subject = (pjsip_subject_hdr*)pjsip_msg_find_hdr_by_name(tdata->msg, &gSubjectHdr, NULL);
	priority = (pjsip_priority_hdr*)pjsip_msg_find_hdr_by_name(tdata->msg, &gPriorityHdr, NULL);

	if (subject != NULL && priority != NULL)
	{
		pjsip_subject_hdr* temp_hdr = (pjsip_subject_hdr*)pjsip_hdr_clone(inv->pool, subject);
		if (temp_hdr != NULL)
		{
			pj_memcpy(&inv->subject, temp_hdr, sizeof(pjsip_subject_hdr));
		}
		temp_hdr = (pjsip_subject_hdr*)pjsip_hdr_clone(inv->pool, priority);
		if (temp_hdr != NULL)
		{
			pj_memcpy(&inv->priority, temp_hdr, sizeof(pjsip_subject_hdr));
		}
	}
	else
	{
		pj_memset(&inv->subject, 0, sizeof(pjsip_subject_hdr));
		pj_memset(&inv->priority, 0, sizeof(pjsip_priority_hdr));
	}
...
pj_bool_t pjsua_call_on_incoming(pjsip_rx_data *rdata)
{
	...
	{		
		...
		*/
		if (response) 
		{
			...
			pjsip_endpt_send_response(pjsua_var.endpt, &res_addr, response, NULL, NULL);
	...
	*/
	...
	{
		status = pjmedia_sdp_parse(rdata->tp_info.pool, 	(char*)rdata->msg_info.msg->body->data, 	rdata->msg_info.msg->body->len, &offer);
		if (status == PJ_SUCCESS) 
		{
			/* Validate */
	...
	/* Init media channel */
	status = pjsua_media_channel_init(call->index, PJSIP_ROLE_UAS, call->secure_level, 	rdata->tp_info.pool, offer, 	&sip_err_code);
	if (status != PJ_SUCCESS) 
	{
		pjsua_perror(THIS_FILE, "Error initializing media channel", status);
		pjsip_endpt_respond(pjsua_var.endpt, NULL, rdata, sip_err_code, NULL, NULL, NULL, NULL);
		PJSUA_UNLOCK();
		return PJ_TRUE;
	}
	...
	{
		status = pjsua_acc_create_uas_contact(rdata->tp_info.pool, &contact, acc_id, rdata);
		if (status != PJ_SUCCESS) 
		{
			pjsua_perror(THIS_FILE, "Unable to generate Contact header", 
				status);
	...
	if (status != PJ_SUCCESS) 
	{
		/** 20160602. AGL. Cambio 500 por 400 */
	...
	if (status != PJ_SUCCESS) 
	{
		...
		w = pjsip_warning_hdr_create_from_status(dlg->pool, 	pjsip_endpt_name(pjsua_var.endpt),	status);
		pj_list_init(&hdr_list);
		pj_list_push_back(&hdr_list, w);
	...
	*/
	...
	if (inv->subject.hvalue.slen == 0 && inv->priority.hvalue.slen == 0)
	{
		subject = (pjsip_subject_hdr*)pjsip_msg_find_hdr_by_name(rdata->msg_info.msg, &gSubjectHdr, NULL);
		priority = (pjsip_priority_hdr*)pjsip_msg_find_hdr_by_name(rdata->msg_info.msg, &gPriorityHdr, NULL);
	...
	//} else {
		...
		*/
		if (pjsua_var.ua_cfg.cb.on_incoming_call) {
			pjsua_call_id replaced_call_id = PJSUA_INVALID_ID;

			if (replaced_dlg != NULL) {
				pjsua_call *replaced_call = (pjsua_call*) replaced_dlg->mod_data[pjsua_var.mod.id];
				replaced_call_id = replaced_call->index;
			}

			pjsua_var.ua_cfg.cb.on_incoming_call(acc_id, call_id, replaced_call_id, rdata);
		} else {
													 ...
													 pjsua_call_info *info)
{
	...
	{
		info->local_focus = call->inv->dlg->local.contact->focus;
		info->remote_focus = (call->inv->dlg->remote.contact && call->inv->dlg->remote.contact->focus);
	}
													 ...
													 const pjsua_msg_data *msg_data)
{
	...
	prev_local_hold = call->local_hold;
	call->local_hold = PJ_TRUE;

	status = create_sdp_of_call_hold(call, &sdp);
	if (status != PJ_SUCCESS) {
		call->local_hold = prev_local_hold;
		pjsip_dlg_dec_lock(dlg);
		return status;
	}
												...
												const pjsua_msg_data *msg_data)
{
	...
	pj_status_t status;
	pjsip_uri *referred_uri;
	char referred_by_arr[128];
	pj_str_t referred_by_st;
	...
	*/
	...
	referred_uri = pjsip_parse_uri(dlg->pool, 
					   dlg->local.info_str.ptr, 
					   dlg->local.info_str.slen, 0);

	referred_by_st.slen = pjsip_uri_print(PJSIP_URI_IN_FROMTO_HDR, referred_uri, referred_by_arr, sizeof(referred_by_arr));
	referred_by_st.ptr = referred_by_arr;
...
/*
* Initiate attended call transfer to the specified address. 
* Se agrega display name en el refer to
*/
PJ_DEF(pj_status_t) pjsua_call_xfer_replaces_dispname( pjsua_call_id call_id, 
															pjsua_call_id dest_call_id,
															char *referto_display_name,
															unsigned options,
															const pjsua_msg_data *msg_data)
{
	pjsua_call *dest_call;
	...
	pjsip_uri *uri;
	pj_status_t status;
	pj_ssize_t referto_display_name_len;
	...
	*/
	...
	/* Make sure we have sufficient buffer's length */
	if (referto_display_name != NULL)
	{
		referto_display_name_len = strlen(referto_display_name);
	}
	else
	{
		referto_display_name_len = 0;
	}

	PJ_ASSERT_RETURN( 
		referto_display_name_len + 1 +
		dest_dlg->remote.info_str.slen +
		dest_dlg->call_id->id.slen +
		dest_dlg->remote.info->tag.slen +
		dest_dlg->local.info->tag.slen + 32 
		< (long)sizeof(str_dest_buf), PJSIP_EURITOOLONG);
	...
	if (referto_display_name != NULL)
	{
		if (referto_display_name_len > 0)
		{
			strncpy(str_dest_buf, referto_display_name, sizeof(str_dest_buf)-1);
			str_dest_buf[sizeof(str_dest_buf)-1] = '\0';			
			strncat(str_dest_buf, " ", sizeof(str_dest_buf)-1);
			str_dest_buf[sizeof(str_dest_buf)-1] = '\0';
		}
		else
		{
	...
	str_dest.slen = strlen(str_dest_buf);

	uri = (pjsip_uri*)pjsip_uri_get_uri(dest_dlg->remote.info->uri);	
	if (uri == NULL)
	{
		uri = (pjsip_uri*)pjsip_uri_get_uri(dest_dlg->target);
	}	

	len = pjsip_uri_print(PJSIP_URI_IN_REQ_URI, uri, 
		str_dest_buf+str_dest.slen, sizeof(str_dest_buf)-1);
	if (len < 0)
	{
		pjsip_dlg_dec_lock(dest_dlg);
		return PJSIP_EURITOOLONG;
	...
	if (len > 0 && len <= (int)sizeof(str_dest_buf)-str_dest.slen)
	{ }
	else
	{
		pjsip_dlg_dec_lock(dest_dlg);
									 ...
									 int code )
{
		...
		code==PJSIP_SC_NOT_ACCEPTABLE_HERE) 
	{
		...
		if (status == PJ_SUCCESS) {
			if (pj_app_cbs.on_create_sdp)
				pj_app_cbs.on_create_sdp(NULL, call->index, local_sdp, NULL);
													...
													pj_status_t status)
{
	...
	unsigned listened_ports[PJSUA_MAX_CONF_PORTS];			//Puertos de la conferencia que estan escuchandose
	unsigned listened_ports_cnt = 0;
	int returned_code = PJSIP_SC_OK;	

	PJSUA_LOCK();

	call = (pjsua_call*) inv->dlg->mod_data[pjsua_var.mod.id];

	if (status != PJ_SUCCESS && status != PJMEDIA_SDPNEG_ENOMEDIA)
	{
	...
	if (call->monitoring && call->session)
	{
		/* Por aqui pasa cuando se produce un re-invite. Cuando se trata de una sesion del tipo Monitoring (escucha),
		cuyos parametros de la media no van a ser modificados (no se va a aparcar ni nada parecido),
		antes de que se corte la media para la negociacion, guardamos en un array los ports de la
		pjsua que estan siendo escuchados por el port de la sesion del tipo monitoring (escucha). 

		Y al terminar esa negociacion, que en el caso de la sesion del tipo monitoring los parametros de la media
		no van a ser modificados, volvemos a conectar esos ports con el de escucha*/

		pj_status_t status, status2;
		unsigned i;
		pjsua_conf_port_info mon_port_info;

		status = pjsua_enum_conf_ports(port_ids, &ports_count);
		status2 = pjsua_conf_get_port_info(call->conf_slot, &mon_port_info);
		if (status == PJ_SUCCESS && status2 == PJ_SUCCESS)
		{
			for (i = 0; i < ports_count; i++)
			{
				...
				if (status == PJ_SUCCESS && port_ids[i] != call->conf_slot)
				{
					for (j = 0; j < port_info.listener_cnt; j++)
					{
						if (port_info.listeners[j] == mon_port_info.slot_id)
						{
							//El audio de esto port esta enviandose a la escucha
							//Al final estos puertos tienen que volver a conectarse
							if (listened_ports_cnt < PJSUA_MAX_CALLS)
							{
								listened_ports[listened_ports_cnt] = port_info.slot_id;
								listened_ports_cnt++;
							}
						}
					}
	...
	if (pjsip_inv__is_re_invite_radio_type(call->inv) == PJ_FALSE)
	{
		/*En el caso de que sea un re-invite de tipo radio entonces no se llama a 
		pjsua_media_channel_update, porque dicha funcion lo que hace es eliminar la sesion actual de
		media a crear otra nueva, en el caso de la radio eso no es necesario*/
	...
	{
		status = pjsua_media_channel_update_radio_reinvite(call->index, local_sdp, remote_sdp);
		if (status != PJ_SUCCESS)
		{
			pjsua_perror(THIS_FILE, "Unable to modify media session after pjsua_media_channel_update_radio_reinvite",
				status);
			call_disconnect(inv, PJSIP_SC_NOT_ACCEPTABLE_HERE);
			/* No need to deinitialize; media will be shutdown when call
			* state is disconnected anyway.
	...
	if (pjsua_var.ua_cfg.cb.on_call_media_state)
		pjsua_var.ua_cfg.cb.on_call_media_state(call->index, &returned_code);

	if (returned_code >= PJSIP_SC_BAD_REQUEST)
	{
		pjsua_perror(THIS_FILE, "Unable to create media session",
			status);
		call_disconnect(inv, returned_code);
		/* No need to deinitialize; media will be shutdown when call
		* state is disconnected anyway.
		*/
		/*pjsua_media_channel_deinit(call->index);*/
	...
	if (call->monitoring && call->session)
	{
		//Volvemos a conectar los ports que se estaban escuchando
		unsigned i;
		for (i = 0; i < listened_ports_cnt; i++)
		{
			pjsua_conf_connect(listened_ports[i], call->conf_slot);
		}
	}
											  ...
											  const pjmedia_sdp_session *offer)
{
	...
	if (call->local_hold) {
		...
		* (See RFC 3264 Section 8.4 and RFC 4317 Section 3.1)
		*/
		attr = pjmedia_sdp_attr_create(call->inv->pool_prov, "inactive", NULL);
										 ...
										 pjsip_rx_data *rdata )
{
	...
	const pj_str_t str_refer_sub = { "Refer-Sub", 9 };
	const pj_str_t str_ref_by = { "Referred-By", 11 };
	pjsip_generic_string_hdr *refer_to = NULL;
	pjsip_generic_string_hdr *refer_sub = NULL;
	pjsip_generic_string_hdr *refer_by = NULL;
	pj_bool_t no_refer_sub = PJ_FALSE;
	int code = PJSIP_SC_TRYING;
	...
	if (pjsua_var.ua_cfg.cb.on_call_transfer_request == NULL) {
		PJ_LOG(4,(THIS_FILE, "Received REFER without callback!"));
		code = PJSIP_SC_NOT_IMPLEMENTED;
		goto on_return;
	}
	...
	/* Find optional Refer-Sub header */
	refer_sub = (pjsip_generic_string_hdr*)pjsip_msg_find_hdr_by_name(rdata->msg_info.msg, &str_refer_sub, NULL);
	if (refer_sub) {
		if (!pj_strnicmp2(&refer_sub->hvalue, "true", 4)==0)
			no_refer_sub = PJ_TRUE;
	}

	/* Find optional Referred-By header (to be copied onto outgoing INVITE request. */
	refer_by = (pjsip_generic_string_hdr*)pjsip_msg_find_hdr_by_name(rdata->msg_info.msg, &str_ref_by, NULL);

	if (no_refer_sub) {
		const pj_str_t str_false = { "false", 5};

		/* Add Refer-Sub header */
		hdr = (pjsip_hdr*)pjsip_generic_string_hdr_create(inv->dlg->pool, &str_refer_sub, &str_false);
		pj_list_push_back(&hdr_list, hdr);

	} else {
		/* Create transferee event subscription */
		status = pjsip_xfer_create_uas(inv->dlg, NULL, rdata, &sub);
		if (status != PJ_SUCCESS) {
			pjsua_perror(THIS_FILE, "Unable to create xfer uas", status);
		...
		*/
		if (refer_sub) {
			...
			hdr = (pjsip_hdr*)pjsip_generic_string_hdr_create(inv->dlg->pool, &str_refer_sub, &str_true);
			pj_list_push_back(&hdr_list, hdr);
		}

		/* Add expires header: */
		hdr = (pjsip_hdr*)pjsip_msg_find_hdr(rdata->msg_info.msg, PJSIP_H_EXPIRES, NULL);
		if (hdr)
			pj_list_push_back(&hdr_list, pjsip_hdr_shallow_clone(inv->dlg->pool, hdr));
	}
...
on_return:
	...
	if (status != PJ_SUCCESS) {
		pjsua_perror(THIS_FILE, "Unable to create response to REFER", status);
		if (sub)
	...
	status = pjsip_dlg_send_response(inv->dlg, pjsip_rdata_get_tsx(rdata), tdata);
	if (status != PJ_SUCCESS) {
		pjsua_perror(THIS_FILE, "Unable to send response to REFER", status);
		pjsip_tx_data_dec_ref(tdata);
		if (sub)
			pjsip_evsub_terminate(sub, PJ_FALSE);
	...
	if (code == PJSIP_SC_TRYING) {
		pjsua_call * existing_call = (pjsua_call*) inv->dlg->mod_data[pjsua_var.mod.id];

		/* Notify callback */
		if (pjsua_var.ua_cfg.cb.on_call_transfer_request)
			(*pjsua_var.ua_cfg.cb.on_call_transfer_request)(existing_call->index,
					&refer_to->hvalue, refer_by != NULL ? &refer_by->hvalue : NULL,
					&(pjsip_rdata_get_tsx(rdata)->transaction_key), tdata, sub);
	}
	else {
		pjsip_tx_data_dec_ref(tdata);
		if (sub)
...
PJ_DEF(pj_status_t) pjsua_call_transfer_answer(unsigned code, const pj_str_t * tsxKey, 
															  pjsip_tx_data * tdata, pjsip_evsub * sub)
{
	pjsip_transaction * tsx;
	pjsip_dialog *dlg;
	pj_status_t status;

	tsx = pjsip_tsx_layer_find_tsx(tsxKey, PJ_TRUE);
	if (!tsx) {
		PJ_LOG(3,(THIS_FILE, "Unable to get tsx for response to REFER"));
		pjsip_tx_data_dec_ref(tdata);
		return PJSIP_ESESSIONTERMINATED;
	}
	...
	if (!dlg)
	{
		PJ_LOG(3,(THIS_FILE, "Unable to get dlg for response to REFER"));
		pjsip_tx_data_dec_ref(tdata);
	...
	status = pjsip_dlg_modify_response(dlg, tdata, code, NULL);
	if (status != PJ_SUCCESS) {
		pjsua_perror(THIS_FILE, "Unable to set code for response to REFER", status);
		pjsip_tx_data_dec_ref(tdata);
		goto on_return;
	}
...
on_return:
	...
	if (sub)
	{
		if (code < 300)
		{
			pjsua_call_transfer_notify(PJSIP_SC_TRYING, PJSIP_EVSUB_STATE_ACTIVE, sub);
		}
		else
		{
			pjsip_evsub_terminate(sub, PJ_FALSE);
...
PJ_DEF(pj_status_t) pjsua_call_transfer_notify(unsigned code, pjsip_evsub_state state, pjsip_evsub * sub)
{
	pjsip_tx_data * ndata;
	pj_status_t status;
	...
	/* Create initial NOTIFY request */
	status = pjsip_xfer_notify(sub, state, code, NULL, &ndata);
	if (status != PJ_SUCCESS) {
		pjsua_perror(THIS_FILE, "Unable to create NOTIFY to REFER", status);
		return status;
	}

	status = pjsip_xfer_send_request(sub, ndata);
	if (status != PJ_SUCCESS) {
		pjsua_perror(THIS_FILE, "Unable to send NOTIFY to REFER", status);
		return status;
	}
														  ...
														  pjsip_event *e)
{
	...
	else if (tsx->role == PJSIP_ROLE_UAC && tsx->method.id == PJSIP_CANCEL_METHOD && 
		tsx->state == PJSIP_TSX_STATE_COMPLETED && tsx->status_code == PJSIP_SC_CALL_TSX_DOES_NOT_EXIST)
	{
		pjsip_inv_terminate(inv, PJSIP_SC_CALL_TSX_DOES_NOT_EXIST, PJ_TRUE);
	}
...
PJ_DEF(pj_status_t) pjsua_call_set_monitoring(pjsua_call_id call_id, pj_bool_t monitoring)											
{
	pjsua_call *call;
	pjsip_dialog *dlg;
	pj_status_t status;
	...
	status = acquire_call("pjsua_call_set_monitoring()", call_id, &call, &dlg);
	if (status != PJ_SUCCESS)
		return status;
...
static void pjsua_call_on_reinvite_received(pjsip_inv_session* inv, const pjsip_event* e, pjsip_tx_data* tdata, int* returned_code, pj_str_t** cause)
{
	...
	if (pjsua_var.ua_cfg.cb.on_pjsua_reinvite_received) {
		(*pjsua_var.ua_cfg.cb.on_pjsua_reinvite_received)(call->index, e, tdata, returned_code, cause);
	}
...
static pj_status_t pjsua_call_on_send_ack(pjsip_inv_session* inv, pjsip_rx_data* rdata, pjsip_event* e)
{
	...
	if (pjsua_var.ua_cfg.cb.on_pjsua_send_ack)
	{
		status = (*pjsua_var.ua_cfg.cb.on_pjsua_send_ack)(call->index, inv, rdata, e);
	}
