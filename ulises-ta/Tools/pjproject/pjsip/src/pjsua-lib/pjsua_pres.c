...
static pj_bool_t pres_on_rx_request(pjsip_rx_data *rdata)
{
#if PJ_PRESENCE_EVENT_SERVER_ENABLED != 0
    int acc_id;
    pjsua_acc *acc;
    pj_str_t contact;    
    pjsua_srv_pres *uapres;
    ...
    /* Incoming SUBSCRIBE: */
	 ...
	 if (!event || (pj_stricmp(&event->event_type, &STR_PRESENCE) != 0))
		 return PJ_FALSE;

#if PJ_PRESENCE_EVENT_SERVER_ENABLED == 0
	 /*En el caso de los agentes de Ulises, rechazaremos los SUBSCRIBEs al evento de presencia
	 para que no se confunda con el servidor de presencia de verdad*/
	optext = pj_str("Presence Subscriptions are not supported");
	pjsip_endpt_respond_stateless(pjsua_var.endpt, rdata, 
				      PJSIP_SC_BAD_EVENT, &optext,
				      NULL, NULL);
	return PJ_TRUE;
...
static pj_status_t send_publish(int acc_id, pj_bool_t active)
{
	...
	char *start_uri = NULL;
	pj_str_t st_SIPURI = pj_str("sip:");
	pj_str_t st_PRESURI = pj_str("pres:");
	pj_str_t st_TELURI = pj_str("tel:");
	char *epos = NULL;
	...
	/*busca el comienzo de la sip uri, pres uri o tel uri. Para no coger el display name*/
	start_uri = pj_strstr(&acc_cfg->id, &st_SIPURI);
	if (start_uri == NULL)
	{
		start_uri = pj_strstr(&acc_cfg->id, &st_PRESURI);
	}
	if (start_uri == NULL)
	{
		start_uri = pj_strstr(&acc_cfg->id, &st_TELURI);
	}
	if (start_uri == NULL)
	{
	...
	entity.ptr = start_uri;
	entity.slen = acc_cfg->id.slen - (start_uri-acc_cfg->id.ptr);

	/*Nos aseguramos que no contiene ">" al final*/
	epos = pj_strchr(&acc_cfg->id, '>');
	if (epos != NULL)
	{
		if (epos-start_uri < 7)
		{
...
static void refresh_timer_cb( pj_timer_heap_t *timer_heap,
				   struct pj_timer_entry *entry)
{
	...
	pjsip_from_hdr* from = NULL;
    
    PJ_UNUSED_ARG(timer_heap);

	from = pjsip_publishc_get_from_hdr(pubc);
	if (from != NULL)
	{
		char buff[64];
	...
	{
		//Con esta funcion nos aseguramos que se envia el publish con el cuerpo xml
		pjsua_acc *acc = &pjsua_var.acc[acc_id];
		if (acc)
		{
			if (acc->online_status != 0 && acc->publish_sess) 
			{
...
pj_status_t pjsua_pres_init_publish_acc(int acc_id)
{
	...
	status = pjsip_publishc_create(pjsua_var.endpt, &acc_cfg->publish_opt, 
				       acc, &publish_cb, &refresh_timer_cb,
				       &acc->publish_sess);
	if (status != PJ_SUCCESS) {
	    acc->publish_sess = NULL;
	    return status;
	...
	else if (pj_strlen(&acc->cfg.reg_uri) > 0)
	{
		...
		route = (pjsip_route_hdr *) pjsip_parse_hdr( acc->pool, &hname, 
						uri, strlen(uri),
						NULL);
		if (route != NULL)
			pj_list_push_back(&route_set, route);

		pjsip_publishc_set_route_set(acc->publish_sess, &route_set);
