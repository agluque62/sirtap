/* $Id: pjsua_media.c 2758 2009-06-12 11:15:08Z nanang $ */
/* 
			    ...
			    pj_status_t result)
{
	...
	if (result != PJ_SUCCESS) {
		...
		pjsua_var.ua_cfg.cb.on_call_media_state(id, NULL);
	    }
	} else {
					   ...
					   int *sip_status_code)
{
	...
	if (pj_app_cbs.on_create_sdp)
		pj_app_cbs.on_create_sdp(pool, call_id, sdp, call->incoming_rdata);
				       ...
				       const pjmedia_sdp_session *remote_sdp)
{
	...
	pjmedia_session_info sess_info_prov;
    pjmedia_stream_info *si = NULL;
    pjmedia_port *media_port;
    pj_status_t status;
	pj_bool_t is_radio_session = PJ_FALSE;
	...
	status = pjmedia_session_get_info(call->session, &sess_info_prov);
	if (status == PJ_SUCCESS)
	{
		if (sess_info_prov.stream_cnt == 1 && sess_info_prov.stream_info[0].ka_type == PJMEDIA_STREAM_KA_R2S)
		{
			//Es una sesion de radio ya establecida. 
			is_radio_session = PJ_TRUE;
...
pj_status_t pjsua_media_channel_update_radio_reinvite(pjsua_call_id call_id,
	const pjmedia_sdp_session* local_sdp,
	const pjmedia_sdp_session* remote_sdp)
{
	pjsua_call* call = &pjsua_var.calls[call_id];
	...
	PJ_ASSERT_RETURN(is_radio_session, PJ_EBUG);
	...
	status = pjmedia_radio_session_modify(call->session, &sess_info);
	return status;	
}
				    ...
				    unsigned bits_per_sample)
{
    ...
    param->in_channel_count = channel_count;
	 param->out_channel_count = channel_count;
	 param->samples_per_frame = samples_per_frame;
    param->in_samples_per_frame = samples_per_frame;
	 param->out_samples_per_frame = samples_per_frame;
