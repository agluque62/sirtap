/* $Id: pjsua_acc.c 2745 2009-06-04 15:51:39Z bennylp $ */
/* 
...
PJ_DEF(pj_status_t) pjsua_acc_del(pjsua_acc_id acc_id)
{
	...
	pjsua_var.acc[acc_id].cfg.publish_enabled = PJ_FALSE;		
...
static void regc_cb(struct pjsip_regc_cbparam *param)
{
	...
	(param->code == PJSIP_SC_REQUEST_TIMEOUT ||
	 ...
	 param->code == PJSIP_SC_FORBIDDEN ||			//MODIF EDUARDO 02/12/2015
	 param->code == PJSIP_SC_UNAUTHORIZED ||		//MODIF EDUARDO 27/12/2015. No quiero que dejen de intentar registrarse.
 ...
 */
PJ_DEF(pjsua_acc_id) pjsua_acc_find_for_incoming_by_uri(pjsip_uri *uri)
{
	pjsip_sip_uri *sip_uri;
	unsigned i;
 ...
 * Returns the account. An invalid id is returned if an account is not found
 */
PJ_DEF(pjsua_acc_id) pjsua_acc_find_for_incoming_by_uri_case_sensitive(pjsip_uri* uri)
{
	pjsip_sip_uri* sip_uri;
	unsigned i;
	...
	for (i = 0; i < pjsua_var.acc_cnt; ++i) {
		...
		if (acc->valid && pj_strcmp(&acc->user_part, &sip_uri->user) == 0 &&
			pj_strcmp(&acc->srv_domain, &sip_uri->host) == 0)
		{
			/* Match ! */
			PJSUA_UNLOCK();
			return acc_id;
	...
	for (i = 0; i < pjsua_var.acc_cnt; ++i) {
		...
		if (acc->valid && pj_strcmp(&acc->srv_domain, &sip_uri->host) == 0) {
			/* Match ! */
			PJSUA_UNLOCK();
			return acc_id;
		}
...
/*
 * This is an internal function to find an account by a user and domain of uri case sensitive
 * Returns the account. An invalid id is returned if an account is not found
 */
PJ_DEF(pjsua_acc_id) pjsua_acc_find_for_incoming_by_user_and_domain_or_transport_case_sensitive(pjsip_uri* uri)
{
	pjsip_sip_uri* sip_uri;
	unsigned i;
...
PJ_DEF(pjsua_acc_id) pjsua_acc_find_for_incoming(pjsip_rx_data *rdata)
{
	...
	ret = pjsua_acc_find_for_incoming_by_uri(uri);
						  ...
						  const pj_str_t * params)
{
	...
	*contact = acc->cfg.force_contact;
	...
	else if (PJSIP_URI_SCHEME_IS_SIPS(sip_uri))
     ...
     * to use IPv6 as well.
     */
	if (sip_uri && pj_strchr(&sip_uri->host, ':'))
...
static void auto_rereg_timer_cb(pj_timer_heap_t *th, pj_timer_entry *te)
{
	...
	PJ_LOG(5,(__FILE__, "RE-REGISTRATION attempt_cnt %d acc->valid %d acc->auto_rereg.active %d", 
		acc->auto_rereg.attempt_cnt, acc->valid, acc->auto_rereg.active));
