...
PJ_DECL(pj_status_t) pjsip_evsub_unregister_pkg(pjsip_module* pkg_mod, const pj_str_t* event_name);
 ...
 * @return		
 */
PJ_DECL(void) pjsip_evsub_set_state(pjsip_evsub *sub, pjsip_evsub_state state);
...
/**
 * Reject the incoming subscription request 
 *
 * @param sub		Server subscription instance.
 * @param rdata		The incoming subscription request message.
 * @param st_code	Status code, which MUST be final response.
 ...
 * @return		PJ_SUCCESS on success.
 */
PJ_DECL(pj_status_t) pjsip_evsub_reject(pjsip_evsub * sub,
    pjsip_rx_data * rdata,
    int st_code,
    const pjsip_hdr * hdr_list);
...
/**
 * For notifier, create NOTIFY request to subscriber forcing Subscription-state regardeless the current status, the headers is set depending the parameters, and set the state
 * of the subscription.
 *
 * @param sub		The server subscription (notifier) instance.
 * @param expires	Force The value of header expires. 
 * @param state_str	The force state string name,
 * @param reason	Force the reason
 * @param retry_after Force the Retry-after header value
 * @param p_tdata	Pointer to receive request message.
 *
 * @return		PJ_SUCCESS on success.
 */
PJ_DECL(pj_status_t) pjsip_evsub_notify_force_subs_state(pjsip_evsub* sub,
 ...
 */
PJ_DECL(void) pjsip_evsub_set_user_data(pjsip_evsub *sub, void *user_data);

/*
 * Set maximum_expires_as_uas
 */
PJ_DECL(void) pjsip_evsub_set_maximum_expires_as_uas(pjsip_evsub* sub, int maximum_expires_as_uas);

PJ_DECL(void) pjsip_evsub_set_minimum_expires_as_uas(pjsip_evsub* sub, int minimum_expires_as_uas);

/*
 * Get subscription user data.
 */
PJ_DECL(void *) pjsip_evsub_get_user_data(pjsip_evsub *sub);
 ...
 */
PJ_DECL(void) set_timer_uas_timeout( pjsip_evsub *sub);
 ...
 */
PJ_DECL(void) set_timer_uac_refresh_timeout( pjsip_evsub *sub);

/*
 * Get event subscription's dlg.
 */
PJ_DECL(pjsip_dialog*) pjsip_evsub_get_dlg( pjsip_evsub *sub );

/**
 * Get subscription event header.
 *
 * @param sub		Event subscription instance.
 *
 * @return		Event header.
 */
PJ_DECL(void *) pjsip_evsub_get_event_hdr(pjsip_evsub *sub);

PJ_DECL(pj_pool_t *) pjsip_evsub_get_pool(pjsip_evsub *sub);
 ...
 */
PJ_DECL(pjsip_evsub*) search_subscription_in_dialog(pjsip_dialog* dlg, pj_str_t* event_type);

PJ_DECL(pj_int32_t) pjsip_evsub_get_expires(pjsip_evsub* sub);
