/* $Id: pjsua.h 2762 2009-06-15 16:03:40Z bennylp $ */
/* 
...
#define PJSUA_CALL_CONFERENCE		0x80000000
...
typedef struct pjsua_callback
{
    ...
    void (*on_incoming_call)(pjsua_acc_id acc_id, pjsua_call_id call_id,
			     pjsua_call_id replaced_call_id, pjsip_rx_data *rdata);
     ...
     *
     * @param call_id	The call index.
	 * @param code		Returned code. If success code=200
     */
    void (*on_call_media_state)(pjsua_call_id call_id, int *returned_code);
	...
	void (*on_pjsua_reinvite_received)(pjsua_call_id call_id, const pjsip_event* e, pjsip_tx_data* tdata, int* returned_code, pj_str_t** cause);

    pj_status_t (*on_pjsua_send_ack)(pjsua_call_id call_id, pjsip_inv_session* inv, pjsip_rx_data* rdata, pjsip_event* e);
...
/*
 * This is an internal function to find an account by a uri
 * Returns the account. An invalid id is returned if an account is not found
 */
PJ_DECL(pjsua_acc_id) pjsua_acc_find_for_incoming_by_uri(pjsip_uri *uri);
PJ_DECL(pjsua_acc_id) pjsua_acc_find_for_incoming_by_uri_case_sensitive(pjsip_uri* uri);

/*
 * This is an internal function to find an account by a user of uri case sensitive
 * Returns the account. An invalid id is returned if an account is not found
 */
PJ_DECL(pjsua_acc_id) pjsua_acc_find_for_incoming_by_user_and_domain_or_transport_case_sensitive(pjsip_uri* uri);
...
typedef struct pjsua_call_info
{
	...
	char	last_status_text[128];
	 ...
	 pj_bool_t local_focus;
	 pj_bool_t remote_focus;
 ...
 *			as the target URI).
 * @param options	Options (PJSUA_CALL_CONFERENCE).
...
PJ_DECL(pj_status_t) pjsua_call_transfer_answer(unsigned code, const pj_str_t * tsxKey, 
																pjsip_tx_data * tdata, pjsip_evsub * sub);

PJ_DECL(pj_status_t) pjsua_call_transfer_notify(unsigned code, pjsip_evsub_state state, 
																pjsip_evsub * sub);
...
*/
PJ_DECL(pj_status_t) pjsua_call_xfer_replaces_dispname( pjsua_call_id call_id, 
															pjsua_call_id dest_call_id,
...
PJ_DECL(pj_status_t) pjsua_call_set_monitoring(pjsua_call_id call_id, pj_bool_t monitoring);
...
/**
 * Send instant messaging outside dialog, using the specified account for
 * route set and authentication. No Route header is added
 *
 * @param acc_id	Account ID to be used to send the request.
 * @param to		Remote URI.
 * @param mime_type	Optional MIME type. If NULL, then "text/plain" is 
 ...
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_im_send_no_proxy(pjsua_acc_id acc_id, 
				   const pj_str_t *to,
				   const pj_str_t *mime_type,
				   const pj_str_t *content,
				   const pjsua_msg_data *msg_data,
