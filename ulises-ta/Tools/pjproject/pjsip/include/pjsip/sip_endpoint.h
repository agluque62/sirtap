/* $Id: sip_endpoint.h 3106 2010-02-24 05:43:34Z nanang $ */
/* 
...
/**
 * Modify or add Max-forward header in the list of additional headers to be put in outgoing request message.
 *
 * @param endpt	    The endpoint.
 * @param value	    The Max-forward value
 *
 * @return	   
 */
PJ_DECL(void) pjsip_endpt_modify_max_forward_header(pjsip_endpoint* endpt, pj_int32_t value);

/**
 * Modify options_answer_uri_without_user. 
 *
 * @param endpt	    The endpoint.
 * @param value	    Respuesta al mensaje OPTIONS cuando la URI no incluye user, es decir, es del tipo SIP:192.168.1.200
 *					El valor seria 200 si se quiere contestar un 200 OK, o 404 si se quiere contestar '404 Not found'
 *					Se podria contestar cualquier otro codigo.
 *
 * @return
 */
#define SIP_ENDPOINT_OPTIONS_ANSWERS_DEFAULT (0xFFFF)
PJ_DECL(void) pjsip_endpt_modify_options_answer(pjsip_endpoint* endpt, pj_int32_t value);
PJ_DECL(int) pjsip_endpt_get_options_answer(pjsip_endpoint* endpt);
...
PJ_DECL(void) pjsip_endpt_Set_Ed137_version(pjsip_endpoint* endpt, char ED137Radioversion, char ED137Phoneversion);
...
* @param isETM	Si es true entonces establece que es ETM
*/
PJ_DECL(void) pjsip_endpt_Set_ETM(pjsip_endpoint* endpt, pj_bool_t isETM);

/**
* Retorna si el endpoint es un ETM o no
* @param endpt  Endpoint
* @return PJ_TRUE si es ETM
*/
PJ_DECL(pj_bool_t) pjsip_endpt_Is_ETM(pjsip_endpoint* endpt);
