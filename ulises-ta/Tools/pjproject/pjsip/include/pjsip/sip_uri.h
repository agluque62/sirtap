...
typedef struct pjsip_uri_vptr
{
    ...
    void *(*p_clone)(pj_pool_t *pool, const void *uri);
	 /**
     * Get the URI diaplay name 
     * @param uri the URI (self).
     */
    pj_str_t (*p_get_display)(void *uri);
 ...
 * @return	    the URI.
 */
PJ_INLINE(pj_str_t) pjsip_uri_get_display(const void *uri)
{
	//He visto que p_get_display es el unico elemento de la estructura pjsip_uri_vptr que puede ser NULL
	if (((pjsip_uri*)uri)->vptr->p_get_display == NULL)
	{
		//En este caso retornamos un string vacio
		pj_str_t disp;
		disp.ptr = NULL;
		disp.slen = 0;
		return disp;
	...
	{
		return (*((pjsip_uri*)uri)->vptr->p_get_display)((void*)uri);
	}
}
...
PJ_INLINE(int) pjsip_uri_print(pjsip_uri_context_e context,
			       ...
			       char *buf, pj_size_t size)
{
    return (int) (*((const pjsip_uri*)uri)->vptr->p_print)(context, uri, buf, size);
}
