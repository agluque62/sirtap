 ...
 * que va en todos los request y responses de la transaccion
 * Esta funcion se llama cuando ya esta creada la transaccion, para determinar la version
 *
 * @param tsx	    transaccion
 * @param WG67_version	String de la version
 * @return	    PJ_SUCCESS si no hay error
 */
PJ_DECL(pj_status_t) pjsip_tsx_set_WG67_version(pjsip_transaction* tsx, const char* WG67_version);

/**
 * Esta funcion retorna la cabecera WG67-version
 *
 * @param tsx	    transaccion
 * @return	    Retorna la cabecera o NULL, si todavia no ha sido establecida
 */
