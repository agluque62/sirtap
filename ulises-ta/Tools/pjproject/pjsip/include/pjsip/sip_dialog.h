/* $Id: sip_dialog.h 3068 2010-01-21 10:04:26Z bennylp $ */
/* 
...
struct pjsip_dialog
{
	...
	/*ED137 version*/
	pjsip_generic_string_hdr *WG67_version;			//Tiene el valor de la cabecera WG67-version.
	pj_bool_t WG67_version_is_already_negotiated;	//Vale true si ya ha sido negociada la version
};
...
PJ_DECL(void) pjsip_dlg_destroy(pjsip_dialog *dlg);
...
/**
 * Esta funcion sirve para determinar el string de la cabecera WG67-version
 * que va en todos los request y responses del dialogo
 * Esta funcion se llama cuando ya esta creado el dialogo, para determinar la version
 * Antes de crear el dialogo hay que determinar esta cabecera de otra forma
 *
 * @param dlg	    dialogo
 * @param WG67_version	String de la version
 * @param WG67_version_is_already_negotiated   Si es true la version ya ha sido negociada
 * @return	    PJ_SUCCESS si no hay error
 */
PJ_DECL(pj_status_t) pjsip_dlg_set_WG67_version(pjsip_dialog* dlg, const char* WG67_version, pj_bool_t WG67_version_is_already_negotiated);

/**
 * Esta funcion retorna la cabecera WG67-version
 *
 * @param dlg				dialogo
 * @param WG67_version		wg67-version del dialogo ya asignado
 * @return	    Si es true la version ya ha sido negociada
 */
PJ_DECL(pj_bool_t) pjsip_dlg_get_WG67_version(pjsip_dialog* dlg, pjsip_generic_string_hdr* WG67_version);
