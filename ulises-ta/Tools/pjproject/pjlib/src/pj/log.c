/* $Id: log.c 2394 2008-12-23 17:27:53Z bennylp $ */
/* 
...
#else
static int pj_log_max_level = PJ_MIN(PJ_LOG_MAX_LEVEL, 6);
#endif
...
{
    pj_log_max_level = PJ_MIN(level, 6);
}
		     ...
		     const char *format, va_list marker)
{
    ...
    if (!level || (level > pj_log_max_level))
	return;
	...
	pj_ansi_strcpy(pre, ltexts[level - 1]);
	pre += 6;
    }
