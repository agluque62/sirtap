...
extern BOOL g_bmAgcEn;
extern BOOL g_bmCurrentAgc;
extern BOOL g_bmAaPathEn;
extern BYTE code MixerTable;
...
BYTE code OriginAcUnitTable1[T1_MAX_UNIT_NUM + 1] =
{
	...
	AC_OT, 
	AC_OT,
	(FEATURE_DAC << 4) | AC_FEATURE,	
	(FEATURE_ADC_MIC << 4) | AC_FEATURE,
	(FEATURE_MONITOR_MIC << 4) | AC_FEATURE,
	(1 << 4) | AC_SELECTOR,
  AC_MIXER 
};	
...
AUDIO_CONTROL_STRUCT code OriginAudioControl1 =
{
	...
	OriginRecordMute,
	OriginAcUnitTable1,
	{{0x11, 0x01, OriginSetFrequency1}, 
	 {0x21, 0x82, OriginSetFrequency2}}
};
		...
		case FEATURE_MONITOR_MIC:
			if(g_bCurrentMute & (bmBIT0 << FEATURE_MONITOR_MIC))
			{
				InternSpiWriteByte(ANALOG_CODEC_CTRL_1, 
					InternSpiReadByte(ANALOG_CODEC_CTRL_1) | bmBIT3);			 //to Mute
			}
			else
			{
				InternSpiWriteByte(ANALOG_CODEC_CTRL_1, 
					InternSpiReadByte(ANALOG_CODEC_CTRL_1) & ~bmBIT3);			//to unmute		
			}
			break;
...
void OriginFeatureAgc()
{
	if(g_bCurrentSelector == SELECTOR_MIC)
	{
		if(g_bmCurrentAgc)
		{
			AGC_GLOBAL_CTRL |= (bmBIT5 | bmBIT4);
			AGC_SIM_CTRL |= bmBIT4;		//enable REC clipping led.
		}
		else
		{
			AGC_GLOBAL_CTRL &= ~(bmBIT5 | bmBIT4);
			AGC_SIM_CTRL &= ~bmBIT4;		//disable REC clipping led.
		}
	}		 					
}
...
BOOL tackleMixer(BOOL dataStage)
{
	if(g_UsbCtrlCmnd.bmRequestType & bmBIT7)	// Device to Host
	{
		switch(g_UsbCtrlCmnd.bRequest)
		{
			...
			case CMD_GET_MIN:
			case CMD_GET_MAX:					
				if(MSB(g_UsbCtrlCmnd.wValue) == 0x00)
				{
					if(dataStage == FALSE)
					{
						g_pbDataBuffer = &((BYTE)MixerTable);
						
						if(g_bmAaPathEn)
						{
							if((HI_NIBBLE(BONDING_OPTION) & 0x03) == 0x00)
							{
								g_wDataLength = 16;
							}
							else
				...
				else if(MSB(g_UsbCtrlCmnd.wValue) == 0xFF)
				{
					return FALSE;
				}
				else
				{
					g_wDataLength = 2;
					g_bTemp = ((MSB(g_UsbCtrlCmnd.wValue) - 1) * 2) + (LSB(g_UsbCtrlCmnd.wValue) - 1);
					C2MCopy((BYTE *)g_bDataBuffer, (BYTE code *)((&MixerTable) + g_bTemp * 2), 2);
					g_pbDataBuffer = g_bDataBuffer;	  					
			  	}				  
			  break;
...
BOOL tackleFeatureAGC(BOOL dataStage)
{
	if(g_UsbCtrlCmnd.bmRequestType & bmBIT7)	// Device to Host
	{	
		if(g_UsbCtrlCmnd.bRequest == CMD_GET_CURRENT)
		{
			g_bDataBuffer[0] = g_bmCurrentAgc ? 1 : 0;
			g_wDataLength = 1;
			g_pbDataBuffer = g_bDataBuffer;		
		}
		else
		...
		case AC_FEATURE:
			switch(MSB(g_UsbCtrlCmnd.wValue))
			{
				...
				case AUTOMATIC_GAIN_CONTROL:
					return (tackleFeatureAGC(dataStage));
				default:
					break;
			}
		  break;
