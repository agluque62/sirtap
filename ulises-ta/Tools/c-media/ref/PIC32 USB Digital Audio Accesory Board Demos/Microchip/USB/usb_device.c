            ...
            if(USBTransactionCompleteIF)
            {
                ...
                //than EP0, if ping pong buffering is enabled.
                    ...
                    else
                    {
                        ...
                        handle = (USB_HANDLE)pBDTEntryIn[endpoint_number];
                    }
                ...
                {
                    if ( USBHALGetLastDirection ( USTATcopy ) == IN_TO_HOST )
                    {
                        endpoint_number |= 0x80;
                    }
                    #ifdef USB_ENABLE_DEDICATED_TRANSFER_HANDLER
                        USBCBTransfer ( endpoint_number );
                    #else
                        USB_TRANSFER_COMPLETE_HANDLER ( EVENT_TRANSFER, (BYTE*)&USTATcopy.Val, endpoint_number );
                    #endif
                }
...
//!@#$%    handle->STAT.Val |= _USIE | _DTSEN;
    handle->STAT.Val |= _USIE; //!@#$% must disable DTS for iPhone 3GS / 3G iPod touch
...
void USBCancelIO(BYTE endpoint)
{
    ...
    if ( USBPacketDisable == TRUE )  // make sure it's safe to manipulate BDT entries
    {
        if (( endpoint & USB_IN_EP ) != 0 )  // check direction
        {
            endpoint &= USB_IN_EP ^ 0xFF;    // mask off direction bit

            // Special handling for ping-pong buffering
            #if (( USB_PING_PONG_MODE == USB_PING_PONG__FULL_PING_PONG ) || ( USB_PING_PONG_MODE == USB_PING_PONG__ALL_BUT_EP0 ))

                // recognize buffers states and disarm if needed
                // use UOWN states to understand which buffer will be addressed by SIE upon next transfer
                if ( pBDTEntryIn[endpoint]->STAT.UOWN )
                {
                    advance = FALSE;
                ...
                // synchronize PPB pointer with SIE state
                if ( advance )
                {
                    USBAdvancePingPongBuffer ( &pBDTEntryIn[endpoint] );
                }
            ...
            #else
                pBDTEntryIn[endpoint]->Val &= _DTSMASK; // disarm
                pBDTEntryIn[endpoint]->Val ^= _DTSMASK; // toggle the DTS bit back to pre-armed condition
            #endif
        }
        else
        {
            // Special handling for ping-pong buffering
                ...
                // recognize buffers states and disarm if needed
                // use UOWN states to understand which buffer will be addressed by SIE upon next transfer
                if ( pBDTEntryOut[endpoint]->STAT.UOWN )
                {
                    advance = FALSE;
                    pBDTEntryOut[endpoint]->STAT.Val &= _DTSMASK;
                }

                USBAdvancePingPongBuffer ( &pBDTEntryOut[endpoint] );
            ...
            #else
                pBDTEntryOut[endpoint]->Val &= _DTSMASK; // disarm
                pBDTEntryOut[endpoint]->Val ^= _DTSMASK; // toggle the DTS bit back to pre-armed condition
            #endif
...
static void USBCheckStdRequest(void)
{
    ...
    switch(SetupPkt.bRequest)
    {
        ...
        case USB_REQUEST_SET_INTERFACE:
            ...
            USER_USB_CALLBACK_EVENT_HANDLER ( EVENT_ALT_INTERFACE, 0, ( SetupPkt.bIntfID << 8 ) + SetupPkt.bAltID );
            break;
