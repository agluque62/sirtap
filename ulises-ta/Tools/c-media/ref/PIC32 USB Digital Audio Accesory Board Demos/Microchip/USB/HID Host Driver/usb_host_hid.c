...
{
    BYTE                                state;                 // State of the endpoint.
    ...
    BYTE                                endpoint;              // Endpoint to use for the transfer.
...
#define _USBHostHID_LockDevice(x)                   {                                                   \
    ...
    #define _USBHostHID_TerminateWriteTransfer( error ) {                                                                   \
        ...
        #define _USBHostHID_TerminateWriteTransfer( error ) {                                                                   \
                                                                deviceInfoHID[i].errorCode          = error;                    \
                                                                deviceInfoHID[i].state              = STATE_RUNNING;            \
                                                                deviceInfoHID[i].transferOUTstate   = STATE_TRANSFER_WAITING;   \
                                                            }
    #endif
#endif
                ...
                BYTE size, BYTE *data )
{
    ...
    #ifdef DEBUG_MODE
//        UART2PrintString( "HID: Transfer: Read" );
    #endif
...
//        UART2PutHex( deviceInfoHID[i].endpointIN );
//        UART2PrintString( "\r\n" );
    #endif
...
BYTE USBHostHIDResetDeviceWithWait( BYTE deviceAddress  )
{
    ...
    if (USBHostHIDTransferIsComplete( deviceAddress, USB_HID_TRANSFER_OUT, &errorCode, (BYTE*)&byteCount ))
    {
        return errorCode;
    }
...
void USBHostHIDTasks( void )
{
    ...
    for (i=0; i<USB_MAX_HID_DEVICES; i++)
    {
        ...
        switch (deviceInfoHID[i].state & STATE_MASK)
        {
            ...
            case STATE_RUNNING:
                switch (deviceInfoHID[i].transferIN.state)
                {
                    case STATE_TRANSFER_WAITING:
                        /* waiting for request from application */
                        break;

                    case STATE_TRANSFER_REQUESTED:
                        if (USBHostTransferIsComplete( deviceInfoHID[i].ID.deviceAddress, deviceInfoHID[i].transferIN.endpoint, &errorCode, &byteCount ))
                        {
                            if (errorCode)
                            {
                                if(USB_ENDPOINT_STALLED == errorCode)
                                {
                                     ...
                                     deviceInfoHID[i].transferIN.returnState = STATE_TRANSFER_WAITING;
                                     deviceInfoHID[i].flags.bfReset = 1;
                                     _USBHostHID_ResetStateJump( i );
                                }
                                else
                            ...
                            else
                            {
                                // Clear the STALL.  Since it is EP0, we do not have to clear the stall.
                                USBHostClearEndpointErrors( deviceInfoHID[i].ID.deviceAddress, deviceInfoHID[i].transferIN.endpoint );
                                deviceInfoHID[i].transferIN.bytesTransferred = byteCount; /* Can compare with report size and flag error ???*/
                               deviceInfoHID[i].transferIN.state = STATE_TRANSFER_COMPLETE;
                            }
                        }
                        #ifdef DEBUG_MODE
                            UART2PrintString("|");
                ...
                switch (deviceInfoHID[i].transferOUT.state)
                {
                    case STATE_TRANSFER_WAITING:
                        /* waiting for request from application */
                        break;
                    ...
                    case STATE_TRANSFER_REQUESTED:
                        if (USBHostTransferIsComplete( deviceInfoHID[i].ID.deviceAddress, deviceInfoHID[i].transferOUT.endpoint, &errorCode, &byteCount ))
                        {
                            if (errorCode)
                            {
                                if(USB_ENDPOINT_STALLED == errorCode)
                                {
                                     ...
                                     deviceInfoHID[i].transferOUT.returnState = STATE_TRANSFER_WAITING;
                                     deviceInfoHID[i].flags.bfReset = 1;
                                     _USBHostHID_ResetStateJump( i );
                                }
                                else
                            ...
                            else
                            {
                                ...
                                // Clear the STALL.  Since it is EP0, we do not have to clear the stall.
                                USBHostClearEndpointErrors( deviceInfoHID[i].ID.deviceAddress, deviceInfoHID[i].transferOUT.endpoint);
                                deviceInfoHID[i].transferOUT.state = STATE_TRANSFER_COMPLETE;
                            }
                        }
                        break;
...
BYTE USBHostHIDTerminateTransfer( BYTE deviceAddress, BYTE direction, BYTE interfaceNum )
{
    ...
    if (direction == USB_HID_TRANSFER_OUT)
    {
...
BYTE USBHostHIDTransfer( BYTE deviceAddress, BYTE direction, BYTE interfaceNum, WORD reportid, WORD size, BYTE *data)
{
    if (direction == USB_HID_TRANSFER_OUT)
    {
        return USBHostHIDWrite( deviceAddress, reportid, interfaceNum, size, data );     
    }
    else
    {
        return USBHostHIDRead( deviceAddress, reportid, interfaceNum, size, data );     
    }
}
...
/*******************************************************************************
  Function:
    BOOL USBHostHIDTransferIsComplete( BYTE deviceAddress, BYTE direction, 
...
BOOL USBHostHIDTransferIsComplete( BYTE deviceAddress, BYTE direction, BYTE *errorCode, BYTE *byteCount )
{
    BYTE    i;
                ...
                BYTE size, BYTE *data )
{
    ...
    #ifdef DEBUG_MODE
//        UART2PrintString( "HID: Transfer: Write, " );
    #endif
    ...
    deviceInfoHID[i].transferOUT.reportId          = (reportid |((WORD)USB_HID_OUTPUT_REPORT<<8));
    deviceInfoHID[i].transferOUT.interface         = interface;
    deviceInfoHID[i].errorCode                     = USB_SUCCESS;

    #ifdef DEBUG_MODE
//        UART2PrintString( "Data EP: " );
//        UART2PutHex( deviceInfoHID[i].transferOUT.endpoint );
//        UART2PrintString( "\r\n" );
    #endif
