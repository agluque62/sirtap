...
typedef struct _USB_AUDIO_DEVICE_INFO
{
	...
	BYTE                                interface[USB_NUM_AUDIO_INTERFACES];              // Interface number we are using.
    ...
    BYTE                                endpointAudioStreamIN;    // Streaming audio endpoint.
	BYTE                                endpointAudioStreamOUT;    // Streaming audio endpoint.
        ...
        ISOCHRONOUS_DATA *pIsochronousData )
{
    ...
    if (!USBHostTransferIsComplete( deviceInfoAudioV1[i].ID.deviceAddress, 
            deviceInfoAudioV1[i].endpointAudioStreamOUT, &errorCode, &byteCount ) || 
    ...
    if (!USBHostTransferIsComplete( deviceInfoAudioV1[i].ID.deviceAddress, 
            deviceInfoAudioV1[i].endpointAudioStreamIN, &errorCode, &byteCount ) || 
        deviceInfoAudioV1[i].flags.bfSettingInterface)
    {
        return USB_AUDIO_DEVICE_BUSY;
    }
    ...
    errorCode = USBHostReadIsochronous( deviceInfoAudioV1[i].ID.deviceAddress, 
            deviceInfoAudioV1[i].endpointAudioStreamIN, pIsochronousData );
    if (errorCode)
    {
        #ifdef DEBUG_MODE
...
BYTE USBHostAudioV1TransmitAudioData( BYTE deviceAddress, 
        ISOCHRONOUS_DATA *pIsochronousData, INT dataLength )
{
    DWORD   byteCount;
    BYTE    errorCode;
    BYTE    i;
...
//    }
//    	
//    // Make sure the device is not already transmitting data or setting the interface.
//    if (!USBHostTransferIsComplete( deviceInfoAudioV1[i].ID.deviceAddress, 
//            deviceInfoAudioV1[i].endpointAudioStreamOUT, &errorCode, &byteCount ) || 
//        deviceInfoAudioV1[i].flags.bfSettingInterface)
//    {
...
//
	...
	pIsochronousData->buffers[pIsochronousData->currentBufferUSB/*deviceInfoAudioV1[i].endpointAudioStreamOUT*/].bfDataLengthValid=1;                           
    errorCode = USBHostWriteIsochronous( deviceInfoAudioV1[i].ID.deviceAddress, 
            deviceInfoAudioV1[i].endpointAudioStreamOUT, pIsochronousData );
    if (errorCode)
    {
        #ifdef DEBUG_MODE
            UART2PrintString( "AUD: Cannot start audio stream.\r\n" );
...
BYTE USBHostAudioV1SetInterfaceFullBandwidth( BYTE deviceAddress, BYTE interface )
{
    DWORD   byteCount;
    BYTE    errorCode;
    BYTE    i, j=0;
	...
	for(j=0; j<USB_NUM_AUDIO_INTERFACES; j++){
	    ...
	    errorCode = USBHostIssueDeviceRequest( deviceInfoAudioV1[i].ID.deviceAddress, 0x01, USB_REQUEST_SET_INTERFACE,
	            deviceInfoAudioV1[i].settingFullBandwidth, interface/*deviceInfoAudioV1[i].interface[j]*/, 0, NULL, USB_DEVICE_REQUEST_SET,
...
BYTE USBHostAudioV1SetInterfaceZeroBandwidth( BYTE deviceAddress )
{
	...
	for(j=0; j<USB_NUM_AUDIO_INTERFACES; j++){
	    ...
	    errorCode = USBHostIssueDeviceRequest( deviceInfoAudioV1[i].ID.deviceAddress, 0x01, USB_REQUEST_SET_INTERFACE,
	            deviceInfoAudioV1[i].settingZeroBandwidth, deviceInfoAudioV1[i].interface[j], 0, NULL, USB_DEVICE_REQUEST_SET,
	            deviceInfoAudioV1[i].ID.clientDriverID );
...
BYTE USBHostAudioV1SetSamplingFrequency( BYTE deviceAddress, BYTE *frequency, BYTE endpoint )
{
    BYTE    errorCode;
    BYTE    i;
    ...
    errorCode = USBHostIssueDeviceRequest( deviceInfoAudioV1[i].ID.deviceAddress, 0x22, SET_CUR,
            SAMPLING_FREQ_CONTROL, endpoint, 3, frequency, USB_DEVICE_REQUEST_SET,
            deviceInfoAudioV1[i].ID.clientDriverID );
    if (errorCode)
    {
        #ifdef DEBUG_MODE
...
void USBHostAudioV1TerminateTransfer( BYTE deviceAddress )
{
    ...
    USBHostTerminateTransfer( deviceInfoAudioV1[i].ID.deviceAddress, deviceInfoAudioV1[i].endpointAudioStreamIN );
    USBHostTerminateTransfer( deviceInfoAudioV1[i].ID.deviceAddress, deviceInfoAudioV1[i].endpointAudioStreamOUT );
...
BOOL USBHostAudioV1DataEventHandler( BYTE address, USB_EVENT event, void *data, DWORD size )
{
    ...
    else if (event == EVENT_DATA_ISOC_WRITE)
    {
        return USB_HOST_APP_DATA_EVENT_HANDLER( address, EVENT_AUDIO_STREAM_TRANSMITTED, data, size );
    }
    else
    {
        return FALSE;
    ...
    while (i < ((USB_CONFIGURATION_DESCRIPTOR *)descriptor)->wTotalLength)
    {
        ...
        if (descriptor[i+1] == USB_DESCRIPTOR_INTERFACE)
        {
            ...
            if ((descriptor[i+5] == USB_CLASS_AUDIO) && (descriptor[i+6] == USB_SUBCLASS_AUDIO_STREAMING))
            {
                ...
                deviceInfoAudioV1[device].interface[ifaceIndex++] = descriptor[i+2];
                ...
                else
                {
                    ...
                    while (((deviceInfoAudioV1[device].endpointAudioStreamIN == 0) || (deviceInfoAudioV1[device].endpointAudioStreamOUT == 0)) &&
                           (i < ((USB_CONFIGURATION_DESCRIPTOR *)descriptor)->wTotalLength))
                    {
                        if (descriptor[i+1] == USB_DESCRIPTOR_ENDPOINT)
                        {
                            ...
                            {
	                            if ((descriptor[i+2] & 0x80) == 0x80) // Isochronous
                                	deviceInfoAudioV1[device].endpointAudioStreamIN    = descriptor[i+2];
                                else if ((descriptor[i+2] & 0x80) == 0x00) // Isochronous
                                	deviceInfoAudioV1[device].endpointAudioStreamOUT    = descriptor[i+2];
                ...
                if ((deviceInfoAudioV1[device].flags.bfFoundZeroBandwidthSetting == 1) &&
                    ...
                    ((deviceInfoAudioV1[device].endpointAudioStreamIN != 0) &&
                    (deviceInfoAudioV1[device].endpointAudioStreamOUT != 0)))
                {
    ...
    switch (event)
    {
        ...
        case EVENT_DETACH:           // USB cable has been detached (data: BYTE, address of device)
            ...
            if (i < USB_MAX_AUDIO_DEVICES)
            {
                ...
                deviceInfoAudioV1[i].endpointAudioStreamIN  = 0;
                deviceInfoAudioV1[i].endpointAudioStreamOUT  = 0;
...
//                UART2PrintString( "\r\n" );
            ...
            else if (((HOST_TRANSFER_DATA *)data)->bEndpointAddress == deviceInfoAudioV1[i].endpointAudioStreamIN)
            {
                ...
                USB_HOST_APP_EVENT_HANDLER( i, EVENT_AUDIO_STREAM_RECEIVED, data, size );
            }    
            else if (((HOST_TRANSFER_DATA *)data)->bEndpointAddress == deviceInfoAudioV1[i].endpointAudioStreamOUT)
            {
                // If we received streaming audio data, pass the event up to the application.
                // It's only one more byte of information more than they need (bmAttributes).
                USB_HOST_APP_EVENT_HANDLER( i, EVENT_AUDIO_STREAM_TRANSMITTED, data, size );
            }   
            #endif
            break;
...
//                UART2PrintString( "\r\n" );
            ...
            {
                // Bus error on data stream.
                USB_HOST_APP_EVENT_HANDLER( i, EVENT_AUDIO_STREAM_TRANSMITTED, NULL, 0 );
            }              
            break;
