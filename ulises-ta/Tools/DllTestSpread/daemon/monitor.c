...
static STARTUPINFO siSpreadtest;
static PROCESS_INFORMATION piSpreadtest;
...
static	char		Work_partition_name[MAX_PROCS_RING][MAX_PROC_NAME];
static	status		Work_partition_status[MAX_PROCS_RING];

static short procs_with_problems[MAX_PROCS_RING];
static int num_procs_with_problems = 0;
...
void end_SpreadTest()
{
	CloseHandle(piSpreadtest.hProcess);
	CloseHandle(piSpreadtest.hThread);
}

void run_SpreadTest()
{
	STARTUPINFO siSpreadtest;
	PROCESS_INFORMATION piSpreadtest;
	char cmd[] = "\\Spreadtest\\Spreadtest.exe\"";
	char current_directory[256];
	char cmd_path[256] = "\"";
	if (GetCurrentDirectory(sizeof(current_directory) - strlen(cmd) - 1, current_directory) > 0)
	{
		strcat(cmd_path, current_directory);
		strcat(cmd_path, cmd);
	}
	else
	{
		printf("ERROR. No se puede ejecutar SpreadTest\n");
		return;
	}

	ZeroMemory(&siSpreadtest, sizeof(siSpreadtest));
	siSpreadtest.cb = sizeof(siSpreadtest);
	ZeroMemory(&piSpreadtest, sizeof(piSpreadtest));
	...
	if (!CreateProcess(NULL,   // No module name (use command line)
		...
		&siSpreadtest,            // Pointer to STARTUPINFO structure
		&piSpreadtest)           // Pointer to PROCESS_INFORMATION structure
		)
	{
...
static	void	Test_Spread()
{
	...
	proc	p;
	int	proc_index;
	int	this_proc_index = -1;
	int	i, j, k, h;
	int num_procs;
	SOCKET sock = INVALID_SOCKET;
	WSADATA wsaData = { 0 };
	int iResult = 0;
	short Partition_prev[MAX_PROCS_RING];
	int GlobalStatus_retrans_prev = 0;
	int problems_found = 0;
	int problems_found_total = 0;

	num_procs_with_problems = 0;
	...
	num_procs = Conf_num_procs(&Cn);
	memset(Partition, 0, sizeof(Partition));
	memset(Partition_prev, 0, sizeof(Partition_prev));
	memset(Work_partition_name, 0, sizeof(Work_partition_name));
	for (i = 0; i < Cn.num_segments; i++)
	{
		for (j = 0; j < Cn.segments[i].num_procs; j++)
		{
			proc_id = Cn.segments[i].procs[j]->id;
			proc_index = Conf_proc_by_id(proc_id, &p);
			Partition[proc_index] = 1;
			Partition_prev[proc_index] = 1;
			Work_partition[proc_index] = 1;
			strcpy(Work_partition_name[proc_index], p.name);
		}
	}
	...
	for (i = 0; i < MAX_PROCS_RING; i++)
	{
		if (Partition[i] == 1)
		{
			printf("Index %d proc name %s\n", i, Work_partition_name[i]);
		}
	}
	Send_partition();
	for (j = 5; j > 0; j--)
	...
	for (j = 0; j < Cn.num_segments; j++)
	{
		for (k = 0; k < Cn.segments[j].num_procs; k++)
		{
			...
			addr.sin_port = htons(30008);	// Assign port no to this socket

			proc_id = Cn.segments[j].procs[k]->id;
			proc_index = Conf_proc_by_id(proc_id, &p);
			addr.sin_addr.S_un.S_addr = htonl(p.ifc->ip);
			if (sock != INVALID_SOCKET)
			{
				if (bind(sock, (SOCKADDR*)&addr, sizeof(addr)) == 0)
				{
					this_proc_index = proc_index;
					printf("This node proc_id %d proc_index %d name %s\n", proc_id, proc_index, p.name);
					break;
				}
				else
				{
	...
	for (h = 0; h <= num_procs; h++)
	{
		...
		for (i = 0; i <= num_procs; i++)
		{
			...
			/*Antes de enviar la nueva particion matamos los spreads y esperamos a que se reinicien*/
			printf("--- Se paran los Spreads ---\n");
			Kill_all_spreads();
			for (j = 6; j > 0; j--)
			{
				printf("   \r");
			...
			memset(Partition, 0, sizeof(Partition));
			for (j = 0; j < (num_procs - i); j++)
			{
				if (j != this_proc_index) Partition[j] = 1;
			}

			//Se quitan los procs en los que se han encontrado problemas previamente
			for (j = 0; j < num_procs_with_problems; j++)
			{
				Partition[procs_with_problems[j]] = 0;
			}

			//Siempre esta presente el del puesto donde corre este programa
			if (this_proc_index != -1) Partition[this_proc_index] = 1;

			printf("--- Se envia la particion y se chequea ---\n");

			for (j = 0; j < MAX_PROCS_RING; j++)
			{
				if (Partition[j] == 1)
				{
					printf("Index %d proc name %s\n", j, Work_partition_name[j]);
				}
			}
			...
			if (GlobalStatus.retrans == 0)
			{
				if (GlobalStatus_retrans_prev > 0)
				{
					for (j = 0; j < MAX_PROCS_RING; j++)
					{
						if (Partition_prev[j] == 1 && Partition[j] == 0)
						{
							//Han dejado de haber retransmisiones al quitarse el proc j de la particion
							//Ese proc j tiene problemas

							procs_with_problems[num_procs_with_problems] = j;
							num_procs_with_problems += 1;
						}
					}
				}
			}
			...
			{
				problems_found++;
				problems_found_total++;
			}

			GlobalStatus_retrans_prev = GlobalStatus.retrans;

			for (j = 0; j < MAX_PROCS_RING; j++)
			{
				Partition_prev[j] = Partition[j];
			}
		}
	...
	if (num_procs_with_problems > 0)
	{
		printf("----- PROCS WITH PROBLEMS -----\n");
		for (j = 0; j < num_procs_with_problems; j++)
		{
			proc_index = Conf_proc_by_id(procs_with_problems[j], &p);
			printf("%s, ", p.name);
		}
		printf("\n");
	}
	else if (problems_found_total)
	{
		//Se han encontrado problemas pero no se ha encontrado una transicion de una particion a otra
		//en la que se mejore, entonces este proc tiene problemas
		printf("This position has problems in SPREAD, try run this program in another one\n");
	}
	else
	{
		printf("SPREAD works OK !!!!!!!!!\n");
	}
	...
	printf("\n");
	printf("--- Se reestablece la particion inicial ---\n");
	memset(Partition, 0, sizeof(Partition));
	Send_partition();
	Sleep(2000);
...
/*Esta funcion mata los spreads. Los servicios de MCAST de ULISES los reinicia*/
static	void	Kill_all_spreads()
{
	int16	Kill_partition[MAX_PROCS_RING];
	proc	p;
...
static	void	Report_message(mailbox fd, int dummy, void* dummy_p)
{
	...
	printf("Status at %s index_in_seg %d V%2d.%02d.%2d (state %d, gstate %d) after %d seconds :\n", p.name, p.index_in_seg,
	...
	memcpy(&Work_partition_status[p.index_in_seg], &GlobalStatus, sizeof(GlobalStatus));
...
static void Exit_Usage(const char* exe, const char* msg1, const char* msg2)
{
	...
	Alarm(EXIT,
		...
		"\t-s : Send a status query and show the result\n"
