...
extern char cyyerror[128];			//Array global donde guarda un mensaje de error.
...
int Conf_init(char* file_name, char* my_name, char* error, int size_error)
{
    int ret = 0;
	proc* config_procs;
	...
	if (Config == NULL) {
		snprintf(error, size_error, "Conf_init: Failed to allocate memory for configuration structure\n");
		return -1;
	}
	config_procs = Mem_alloc(MAX_PROCS_RING * sizeof(proc));
	if (config_procs == NULL) {
        dispose(Config);
        snprintf(error, size_error, "Conf_init: Failed to allocate memory for configuration procs array\n");
        return -1;
	}

	Config->allprocs = config_procs;

	ret = Conf_load_conf_file(file_name, my_name, error, size_error);
    if (ret < 0)
    {
        dispose(Config);
        dispose(config_procs);
    }
...
int Conf_load_conf_file(char* file_name, char* my_name, char* error, int size_error)
{
    ...
    if (yyin == NULL)
    {
        snprintf(error, size_error, "Conf_load_conf_file: error opening config file %s\n", file_name);
    ...
    if (strlen(cyyerror) > 0)
    {
        snprintf(error, size_error, "%s", cyyerror);
        return -1;
    }
	...
	if (Config->num_segments > 1) {
		...
		if (found_nonlocal && found_localhost) {
            ...
            snprintf(error, size_error, "Conf_load_conf_file: Localhost segments can not be used along with regular network address segments.\n");
            return -1;
		}
	}
    ...
    if (added_len == -1)
    {
        ...
        snprintf(error, size_error, "Failed to update string with version number!\n");
        return -1;
    }
	...
	if (ConfStringLen >= MAX_CONF_STRING) {
        ...
        snprintf(error, size_error, "Failed to update string with accelerated ring type!\n");
        return -1;
	}
	...
	if (my_name == NULL) {
		...
		if (i < 0) {
            ...
            snprintf(error, size_error, "Conf_load_conf_file: None of local IPs found in configuration\n");
            return -1;
		}
	}
        ...
        if (host_ptr == 0)
        {
            ...
            snprintf(error, size_error, "Conf_load_conf_file: no such monitor host %s\n", machine_name);
            return -1;
        }
        ...
        if (i < 0)
        {
            //Alarm(EXIT, "Conf_load_conf_file: My proc %s is not in configuration \n", My.name);
            snprintf(error, size_error, "Conf_load_conf_file: My proc %s is not in configuration \n", My.name);
            return -1;
        }
	...
	if (PersonalWindow > Window) {
        ...
        snprintf(error, size_error, "Conf_load_conf_file: PersonalWindow (%d) > Window (%d)!\n", PersonalWindow, Window);
        return -1;
	}
	...
	if (AcceleratedRing && AcceleratedWindow > PersonalWindow) {
        ...
        snprintf(error, size_error, "Conf_load_conf_file: AcceleratedWindow (%d) > PersonalWindow (%d)!\n", AcceleratedWindow, PersonalWindow);
        return -1;
	}
