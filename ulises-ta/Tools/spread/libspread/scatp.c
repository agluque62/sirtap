...
long scatp_adv_cpy0(scatp *dst, scatp *src, long num_bytes, int adv_dst, int adv_src) 
{
  ...
  while (dst_elem < dscat->num_elements && src_elem < sscat->num_elements && bytes_left) {
    dst_left  = (long)(dst_end - dst_curr);
    src_left  = (long)(src_end - src_curr);
  ...
  if (adv_dst) {
    ...
    if (dst_elem != dscat->num_elements)
      dst->buff_ind = (long)(dst_curr - dscat->elements[dst_elem].buf);
    else
      dst->buff_ind = 0;
  }
  if (adv_src) {
    ...
    if (src_elem != sscat->num_elements)
      src->buff_ind = (long)(src_curr - sscat->elements[src_elem].buf);
    else
      src->buff_ind = 0;
  }
  return num_bytes;
