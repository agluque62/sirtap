...
void	Conf_load_conf_file( char *file_name, char *my_name )
{
	...
	if (NULL != (yyin = fopen(file_name,"r")) )
                Alarmp( SPLOG_DEBUG, PRINT, "Conf_load_conf_file: using file: %s\n", file_name);
	if (yyin == NULL) 
		if (NULL != (yyin = fopen("./spread.conf", "r")) )
                        Alarmp( SPLOG_DEBUG, PRINT, "Conf_load_conf_file: using file: ./spread.conf\n");
	if (yyin == NULL)
	...
	if( my_name == NULL ){
		...
		name_len = (unsigned)strlen( machine_name );
		if( name_len > sizeof(My.name) ) name_len = sizeof(My.name);
		memcpy(My.name, machine_name, name_len );
		Alarmp( SPLOG_INFO, CONF, "Conf_load_conf_file: My name: %s, id: %d\n",
			My.name, My.id );
		return;
	}else{
		name_len = (unsigned)strlen( my_name );
		if( name_len > sizeof(My.name) ) name_len = sizeof(My.name);
	...
	Alarmp( SPLOG_INFO, CONF, "Conf_load_conf_file: My name: %s, id: %s, port: %hd\n",
		My.name, ip, My.port );
...
{
        if (new_state == FALSE) {
                Alarmp( SPLOG_DEBUG, PRINT, "disabling Dangerous Monitor Commands!\n");
        } else if (new_state == TRUE) {
                Alarmp( SPLOG_DEBUG, PRINT, "ENABLING Dangerous Monitor Commands! Make sure Spread network is secured\n");
        } else {
                /* invalid setting */
                return;
        }
...
void    Conf_set_port_reuse_type(port_reuse state)
{
        ...
        case port_reuse_auto:
                Alarmp( SPLOG_DEBUG, PRINT, "Setting SO_REUSEADDR to auto\n");
                break;
        case port_reuse_on:
                Alarmp( SPLOG_DEBUG, PRINT, "Setting SO_REUSEADDR to always on -- make sure Spread daemon host is secured!\n");
                break;
        case port_reuse_off:
                Alarmp( SPLOG_DEBUG, PRINT, "Setting SO_REUSEADDR to always off\n");
                break;
        default:
...
static void set_param_if_valid(char **param, char *value, char *description, unsigned int max_value_len)
{
        ...
        {
                unsigned int len = (unsigned)strlen(value);
                ...
                *param = buf;
                ...
                Alarmp( SPLOG_DEBUG, PRINT, "Set %s to '%s'\n", description, value);
        }
        else
        {
                Alarmp( SPLOG_DEBUG, DEBUG, "Ignored invalid %s\n", description);
        }
}
