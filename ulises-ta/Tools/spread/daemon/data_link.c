...
channel	DL_init_channel( int32 channel_type, int16 port, int32 mcast_address, int32 interface_address )
{
	...
	if((chan = (int)socket(AF_INET, SOCK_DGRAM, 0)) == -1)
	...
	if ( channel_type & SEND_CHANNEL )
	{
		...
		if (interface_address != 0)
		{
			...
			if (setsockopt(chan, IPPROTO_IP, IP_MULTICAST_IF, (void*)&iface, sizeof(iface)) < 0)
			{
				Alarm( DATA_LINK, "DL_init_channel: setsockopt error for set multicast interface\n");
			}
		}
	...
	if ( channel_type & RECV_CHANNEL )
	{
		...
		if( i1 >=224 && i1 < 240 )
		{
			...
			mreq.imr_interface.s_addr = soc_addr.sin_addr.s_addr; /*INADDR_ANY;*/

        		if (setsockopt(chan, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *)&mreq, 
		       		sizeof(mreq)) < 0) 
			{
				Alarm( EXIT, "DL_init_channel: problem in setsockopt to multicast address %d.%d.%d.%d\n", i1,i2,i3,i4 );
			}
