...
STDINLINE static void stdcarr_low_insert_shift_right(stdcarr *carr, char *it, stdsize delta, stdsize new_size) 
{
  ...
  if (carr->begin <= carr->end) {
    ...
    if ((diff = (stdssize)(carr->end + delta - carr->endbase)) <= 0) { /* no data wraps around */
      memmove(it + delta, it, (stdsize) (carr->end - it));

    } else {

      if ((diff2 = (stdssize)(carr->end - it)) <= diff) { /* data to shift can fit between base and new end */
    ...
    if ((diff = (stdssize)(carr->end - it)) >= 0) {                      /* it is below end */
      memmove(it + delta, it, (stdsize) diff);

    } else {
      memmove(carr->base + delta, carr->base, (stdsize) (carr->end - carr->base));  /* shift lower data */

      if ((stdsize)(diff = (stdssize)(carr->endbase - it)) <= delta) {  /* higher data fits into newly opened area */
...
STDINLINE static void stdcarr_low_insert_shift_left(stdcarr *carr, char *it, stdsize delta, stdsize new_size) 
{
  ...
  if (carr->begin <= carr->end) {
    ...
    if ((diff = (stdssize)(carr->base - (carr->begin - delta))) <= 0) { /* new begin doesn't wrap around */
      memmove(carr->begin - delta, carr->begin, (stdsize) (it - carr->begin));

    } else {

      if ((diff2 = (stdssize)(it - carr->begin)) <= diff) {  /* data to shift fits between new begin and endbase */
    ...
    if (it >= carr->begin) {                               /* it is above begin */
      ...
      if ((stdsize)(diff = (stdssize)(it - carr->base)) <= delta) {    /* fits into newly opened area */
...
STDINLINE static void stdcarr_low_erase_shift_left(stdcarr *carr, char *erase_end, stdsize delta, stdsize new_size) 
{
  ...
  if ((diff = (stdssize)(carr->end - erase_end)) >= 0) {            /* end is at or above erase_end */

    if ((diff2 = (stdssize)(carr->base - erase_begin)) <= 0) {      /* erase region doesn't wrap around */
...
STDINLINE static void stdcarr_low_erase_shift_right(stdcarr *carr, char *erase_begin, stdsize delta, stdsize new_size) 
{
  ...
  if ((diff = (stdssize)(erase_begin - carr->begin)) >= 0) {     /* erase_begin is at or above begin */

    if ((diff2 = (stdssize)(erase_end - carr->endbase)) <= 0) {  /* erase region doesn't wrap around */
    ...
    diff = (stdssize)(erase_begin - carr->base);
    erase_end -= diff;                                             /* move erase_end back diff bytes */
    memmove(erase_end, carr->base, (stdsize) diff);                /* shift lower data first */

    if ((stdsize)(diff = (stdssize)(carr->endbase - carr->begin)) <= delta) { /* fits into newly opened area */
...
{  
  ...
  if (it1->val >= it1->begin) {
    ret = (stdssize)(it2->val >= it1->begin ? it1->val - it2->val : (it1->val - it1->endbase) + (it1->base - it2->val));

  } else {
    ret = (stdssize)(it2->val < it1->begin ? it1->val - it2->val : (it1->endbase - it2->val) + (it1->val - it1->base));
  }

  return (stdssize)(ret / it1->vsize);
}
