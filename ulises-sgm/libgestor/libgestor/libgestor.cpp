...
void Def_Gestor_Terminal::Inicializa_Datos()
{
	...
	int res1=misnmp->get_bulk(pdu,*mictarget,0,50,callback,this);
	if (res1!=SNMP_CLASS_SUCCESS)
...
void Def_Gestor_Terminal::Polling()
{
	...
	// controla)
	// Manda como argumento la propia estructura del gestor
	int res1=misnmp->get(pdu,*mictarget,callbackpolling,this);
	if (res1!=SNMP_CLASS_SUCCESS)
...
#else
	int res1=misnmp->set(pdu,*mictarget);
#endif
	if (res1!=SNMP_CLASS_SUCCESS)
...
#ifndef WIN32
	int res1=misnmp->set(pdu,*mictarget,callback1,this);
#else
	int res1=misnmp->set(pdu,*mictarget);
#endif
	if (res1!=SNMP_CLASS_SUCCESS)
