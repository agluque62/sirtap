...
INSERT INTO OPERADORES ( IdOperador
											 , IdSistema
											 , Clave
											 , NivelAcceso
											 , Nombre
                  ...
                  SELECT IdOperador
											 ...
											 , FechaUltAcceso
											 , Comentarios
										FROM NEW_CD40.OPERADORES
$$

INSERT INTO INCIDENCIAS ( IdIncidencia
												, IdIncidenciaCorrectora
												, Incidencia
												, Descripcion
												, GeneraError
												, OID
                   ...
                   SELECT IdIncidencia
												...
												, OID
												, trep
										FROM NEW_CD40.INCIDENCIAS
$$

INSERT INTO INCIDENCIAS_FRANCES ( IdIncidencia
																, IdIncidenciaCorrectora
																, Incidencia
																, Descripcion
																, GeneraError
													 ...
													 SELECT IdIncidencia
																...
																, OID
																, trep
                             FROM NEW_CD40.INCIDENCIAS_FRANCES
$$

INSERT INTO INCIDENCIAS_INGLES ( IdIncidencia
															 , IdIncidenciaCorrectora
															 , Incidencia
															 , Descripcion
															 , GeneraError
													...
													SELECT IdIncidencia
															 ...
															 , OID
															 , trep
													  FROM NEW_CD40.INCIDENCIAS_INGLES
$$

DROP FUNCTION IF EXISTS GetNombreConf
$$
CREATE FUNCTION GetNombreConf(PConfiguracion varchar(100)) RETURNS VARCHAR(100)
...
$$
CREATE FUNCTION GetNombreConfEdicion(PConfiguracion varchar(100)) RETURNS VARCHAR(100)
...
$$
CREATE PROCEDURE InsertConfiguracionesBD (PConfiguracion_Activa varchar(100))
...
CALL InsertConfiguracionesBD('new_cd40')
$$

DROP PROCEDURE IF EXISTS CheckConfiguracion
$$
CREATE PROCEDURE CheckConfiguracion (INOUT PNombre varchar(100), INOUT PExiste TINYINT(1), INOUT PExisteBD TINYINT(1), INOUT POnline TINYINT(1))
...
$$
CREATE PROCEDURE CheckConfiguracionEdicion (INOUT PNombre varchar(100), INOUT PExiste TINYINT(1), INOUT PExisteBD TINYINT(1), INOUT POnline TINYINT(1))
...
$$
CREATE PROCEDURE AddConfiguracion (PNombre varchar(100), PDescripcion varchar(100), PBorrarSiExiste TINYINT(1))
...
$$
CREATE PROCEDURE DelConfiguracion (PNombre varchar(100))
...
$$
CREATE PROCEDURE DelHistoricos (PNombre varchar(100))
...
$$
CREATE PROCEDURE ActivarConfiguracion (PConfiguracion varchar(100))
...
$$
CREATE PROCEDURE VaciaConfiguracion (PNombre varchar(100))
...
CREATE FUNCTION DiffDescripcionTabla(PConfiguracion VARCHAR(64), PTabla VARCHAR(64)) RETURNS TEXT
...
$$
CREATE PROCEDURE CopyTabla (PConfiguracion varchar(100), PTabla varchar(100))
...
BEGIN
  ...
  -- Comprobamos la descripción de la tabla
	CALL CheckDescripcionTabla(PConfiguracion, PTabla);
...
$$
CREATE PROCEDURE CopyDatosIniciales (PConfDestino varchar(100), PBorrarDatos TINYINT(1))
...
CREATE PROCEDURE CheckForeignKeys(PConfiguracion VARCHAR(64)) 
    LANGUAGE SQL
    NOT DETERMINISTIC
    READS SQL DATA
BEGIN
...
CREATE FUNCTION DiffEsquemaConf(PConfiguracion VARCHAR(64)) RETURNS TEXT
...
CREATE FUNCTION DiffEsquemaConfEdicion(PConfiguracion VARCHAR(64))  RETURNS TEXT
# Comprueba que el modelo de datos es el mismo que el del esquema ONLINE
BEGIN
    DECLARE VEsquemaOnLine      VARCHAR(64);
    DECLARE VEsquema            VARCHAR(64);
...
DROP PROCEDURE IF EXISTS DropTriggers
$$

CREATE PROCEDURE DropTriggers(PEsquema VARCHAR(64)) 
...
BEGIN
    ...
    DECLARE cTriggers CURSOR FOR
            SELECT A.TRIGGER_NAME
             FROM INFORMATION_SCHEMA WHERE TRIGGER_SCHEMA = VEsquema;
